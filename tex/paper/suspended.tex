...
\entry[Terms]{\e}{
  ...
  (e : \t) \andcr
  \erecord { \overline{l = e} } \and
  \efield e l \and
  \exfield e \F l \andcr
   (\eone, \ldots, \en) \and
   \efield e j \and
   \exfield e n j \andcr
   \expoly e { \exi \tvs. \ts } \and
   \einst e \and
   \exinst e \tvs \ts
}\\[1ex]
\entry[Types]{\t}{
...
presentation of \ML differs from most traditional ones that permit type
variables to ``float around''. Instead, we explicitly track the type
variables in scope in the type context $\Gamma$.
...
The typing judgement $\G \th \e : \ts$ states that in context $\G$
expression $\e$ has type scheme $\ts$.  Typing rules are given on \cref
{fig/typing}.

Typing judgments for fully annotated terms as as usual an unsurprising.
However, typing rules for terms with omitted type annotations are
non-composition and are specify in two steps.  We first request a typing
where with an implicit annotation and then show that the annotation is fully
determined from context around the expression. 

We write $\E$ for one-hole expression contexts.  We write $\e^x$ for an
expression with a topmost annotation such as the projection, the creation of
a polytype or the use of a polytype, and $\e^i$ its implicit counterpart
when the annotation has been omitted.

In order to show that $\G \th \E \where {\e^i} \th \t$ we must first show
$\G \th \E \where {e^x} \th \t$ and then show that the context $\E$ and $\E$
and the expression $\e$ taken together ensure that the annotation, \ie the
shape $s$ of the annotation is fully dertermined, either from the expression
$\e$ which we write $\eshape \E \e s$, or from the context $\E$, which we
write $\Eshape \E \e s$.
...
The shape of a non-variable monotype $\tys \F$ is $\F$.
The shape of a variable is undefied (say $\sbot$).
\begin{mathpar}
s \is \exi \tvs \ts \mid \Pi^n \mid \F \mid \sbot
\end{mathpar}
where $\exi \tvs \ts$ is a shape (as defined above).

\Xdidier {We may wish to extend the definition of shapes to all non-variabl
...
In order to describe the judgmnents $\eshape \E \e s$ and
...
determined by the expression $\e$, in the formar case, or by the context
...
Besides, I don't think we can reproduce the examples analoguous to $e_6$ and
$e_7$ with structural tuples.}

The following lemma shows that we can always take a larget context
...
equal to $\efun \x {\eproj \x k}$ is ambigous, \ie that it does not
typecheck.
%
...
\begin{example}
\locallabelreset
We now illustrate a non-ambigous example, showing that the
...
Asume $\eset \th \E \where{\ecast \x \typ \typp} : \t$. Since
...
The following example of non-typability illustratesis how the typing rules
still forces to reject typing of exressions whose annotations would
be unambiguous. This is intended, to prevent us from haveing to focus
