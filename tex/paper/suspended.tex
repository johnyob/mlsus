...
%% article.
\begin{abstract}
  % Traditional type inference algorithms such as $\mathscr{W}$ for Hindley-Milner 
  % or bidirectional typechecking enforce an ordering on the propagation of type information 
  % -- e.g. inferring the type of a let-bound expression before its uses. We revisit the 
  % problem of type inference for several extensions of the ML language, from the constraint-based 
  % perspective in the style of Pottier and R\'emy, to derive a new approach: \textit{omnidirectional type inference}
  % which improves on the state 
