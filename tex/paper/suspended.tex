...
non-composition and specify when an whole expression $\E \where \e^x$ with
an explicitly annotation expression $\e^x$ that is well typed in a context
$\E$ can be omitted turn into an implicitly typed expresion $\e^i$ so that
$\E \where {\e^i}$ is still well-typed. In each case, this requires the
context $\E$ and the expression $\e$ taken together ensure that the
annotation, \ie the shape $s$ of the annotation is fully dertermined, either
from the expression $\e$ which we write $\eshape \E \e s$, or from the
context $\E$, which we write $\Eshape \E \e s$.
...
definition of unavoidable shapes. Finally, we define
\begin{mathpar}
\def \Eqdef {&\eqdef&}
{\begin{tabular}{RCL}
\eshape E \e s \Eqdef
...
The following lemma shows that we can always take the larget context
...
We illustration the typing of implicit constructs with several examples.
\begin{example}
...
A more interesting example of non-typability is how the typing rules forces
us to still reject some examples that have a unique solution.
...
\item
  [$\E$ is to $\elet \x \epid (\eapp {\einst \ehole} 1, \eapp
  {\einst \x} \eunit)$.]
...
\end{mathpar}
However, $\eshape \E \x \tyid$ does not hold. 

\item
  [$\E$ is $\elet \x \epoly {\efun z z} (\eapp {\einst \ehole} 1, \eapp
  {\einst \x} \eunit)$.]  This is symmetric to the previous
  case, which cannot hold either.
