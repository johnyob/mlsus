...
them, such as flexible \Xalistair{structural?} records, higher-kinded types, or dimensional types
fits nicely into the \ML framework, others such as GADTs or \Xalistair{`,' instead of `or'?} semi-explicit
...
omitted and rebuild from context. \Xalistair{We could mention stratified type inference here, 
which makes this idea more explicit}
%
The difficulty lies in finding a methodical such specification \Xalistair{should read as `lies in 
methodically finding such a specifcation ...'?} that comes
...
\Xalistair{I think we could critise bidirectional more? Aside from a forced order, 
it suffers from scalability issues (rule duplication and lack of clear annotability
spec)}
...
\Xalistair{Poly-directional > Let-directional. Let-directional isn't entirely true in 
practice (since annotations are generalizable points). Downside for poly-directional 
is that it sounds like type information can come from many directions?}
...
\end{program}
\Xalistair{We should make it clearer that we're talking about \OCaml with the 
\code{-principal} flag. Since I suspect that many OCaml programmers rely on 
the non-principal directional behaviour (I know I do)}
...
\Xalistair{Algorithm M? I've not heard of this one, do you mean $\mathscr{J}$?} 
...
\begin{enumerate*}
\item
  we first must work with partial types schemes, \ie. \Xalistair{The .,.
  looks like a typography issue? shouldn't it be `\textit{ie.}' } we the ability
...
instance?}
\Xalistair{\cref{sec:solving} is the formal specification of the solver, a small-step operational semantics for constraints, 
\cref{sec:implementation} discusses the implementation of the solver as a big-step operational semantics for constraints.
Also I think there is a fair bit of impressive technical engineering / book-keeping that goes into implementing the solver. }
...
examples, I think the intro has enough. We should put them in the relevant subsections 
in this section.}
...
it is able to track which new nodes need propagating and leaves the rest untouched.}
...
\Xalistair{$\D$ is a list of term \textit{and} type variables. I wouldn't 
say $\D$ is a binder, since it relies on all variables listed being previously 
bound. It simply serves as an explicit closure (which the solver requires to 
track which variables are guarded)}
...
\Xalistair{Fair, I do see the principal types `by construction' part of our work 
as a major improvement over prior art}
These include:
\begin{enumerate}
...
\Xalistair{$\D(\F)(\K)$ looks up the type scheme associated with the constructor $\K$ 
in the definition of $\F$ in the context $\D$ (although this metavar has now been taken 
by closures...) }
