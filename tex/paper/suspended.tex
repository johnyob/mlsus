...
\title{Omnidirectional type inference for fragile \ML type features}
...
types, or dimensional types, fit nicely into the \ML framework. Others such
as GADTs, semi-explicit polymorphism, first-class modules or static
overloading of datatype constructors and record fields, are \emph{fragile},
since they require to twist the ML-style specification with some direct or
indirect algorithmic flavor in order to preserve
\emph{principality}. Fragile conststructs have all in common that their
syntax includes explicit type annotations which may sometimes but not always
be omitted. We call them \emph{fragile} as they have a natural specification
for which they are \emph{not} complete; they are only complete for
a (more or less ad-hoc) restriction of this specification, designed
especially to reject examples that would break completeness, but also many
others.  Unsurprisingly, they have historically posed challenges to the
preservation of principal types.

Fragile constructs can be elaborated into their non-fragile but less
cumbersome to use counterpart, where explicit type annotations are
mandatory. Those fit nicely into the \ML framework. The elaboration then
defines which annotations can be omitted and rebuilt from context, a point
of view already taken in stratified type inference~\cite
