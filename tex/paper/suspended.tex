...
\end{mathpar}
which only applies when $\tau$ already has a shape. In the natural rule we would take the shape of $\semenv{\tau}$, which contains parts that have been ``guess'' from $\semenv$; here the shape must already be known in the constraint.
...
\TODO{Extra examples:
  ...
  \item nested suspended constraints
  \end{itemize}
}
