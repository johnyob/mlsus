...
\begin{mathpar}
\cinfer {\epoly e {\tfor {\overline{\cvb}} \t}} \cva \wide\eqdef
...
\begin{mathpar}
  ...
  \inferrule [Poly]
    {\G \th \e : \sigma[\overline{\tv := \t}] \\
     \G \th \overline{\t}}
    {\G \th \epoly \e { \exists \overline{\tv}. ~ \sigma } :
      [\sigma[\overline{\tv := \t}]]}
...
\end{mathpar}
where $E$ is the term $\efun \x \ehole$.

Hence, if suffices to show that $E \where {\x ! \Pi\iton}$ does not hold.
For that, it suffices to find two derivations of $E \where {\emagic {\x}}$
with different shape for the type of $\x$.  Indeed, contructor $\Pi\iton$
for $n\ge k$ is a possible shape for $n \ge k$.
\end{example}
