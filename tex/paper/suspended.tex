...
\item then we have all the bits to introduce the condition
  ...
  \begin{mathpar}
    \begin{array}{l}
    \exists! \F, \quad \forall \semenv, \semenvp,\\ \qquad \semenv \vdash \cctx[\semenvp \vdash \square] \implies \mathsf{head}(\semenvp(\cv)) = \F
    \end{array}
  \end{mathpar}
\item and finally the global rule
  \begin{mathpar}
    ...
    {\cctx[\cva !]
      ...
      \and \semenv \vdash \cctx[\semenvp, \overline{\cvb := \t} \vdash f(\overline{\cvb} \Fapp)] }
    {\semenv \vdash \cctx[\semenvp \vdash \cmatch \cva \Delta f]}
  \end{mathpar}
\end{itemize}
