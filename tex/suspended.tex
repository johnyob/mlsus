...
\begin{hparens}
\Xalistair{Shapes v3}
A shape $\sh$ is \Xdidier[a \emph{partial type}]{Partial type dpes not mean
anything, syntactically $\t$ is just a type. A partial type would suggest
that it would not be closed. So let's just call it a shape.}:
\begin{mathpar}
  \begin{bnfgrammar}
    \entry[Shape]{\sh}{ \shq \tvcs \t }
  \end{bnfgrammar}
\end{mathpar}
\Xdidier [where $\tvcs$ are bound within the type $\t$.
Shapes must be closed.]{where $\tvcs$ are the free varoables of
$\t$ [This avoids dealing with $\tvc \notin \ftv \t$]}

We consider $\tvcs$ to be flexible within $\t$. The \emph{\Xdidier[bottom]{partilal?}} shape
is defined as $\shq \tvc \tvc$, written $\bot$. For a shape $\shq \tvcs \t$,
we say that $\t$ is the \emph{structure} of the shape.

Shapes are equipped with an ordering---that of the standard
instantiation ordering, defined by:
\begin{mathpar}
  \infer[Inst-Shape]
    {\bar \tvc_2 \disjoint \shq {\tvcs_1} \tone}
    {\shq {\tvcs_1} \tone \preceq \shq {\tvcs_2} \tone \where {\tvcs_1 \is \tys}}
\end{mathpar}
When writing $\sh \preceq \shp$, we say that $\sh$ is more general than $\shp$.

% Define decomposition

For shape $\shq \tvcs \t$, we write $\tys \shapp[\shq \tvcs \t]$ for $\t
\where {\tvcs \is \tys}$.
A type $\t$ has a decomposition $\angles {\sh, \tys}$ iff $\t = \tys \shapp$;
...
\begin{definition}
  ...
  \begin{enumerate}
    \item $\t = \tys \shapp$
    \item For any other non-trivial decomposition $\t = \tys' \shapp[\shp]$, $\sh \preceq \shp$
  \end{enumerate}
\end{definition}
...
\begin{mathpar}
  \all {\text{non-trival } \sh, \shp}
    \sh \preceq \shp \implies f(\head \sh) = f(\head \shp)
\end{mathpar}
\Xdidier {What is the head of a shape (including polytypes), then? Isn't
going to dupplicate/mimic the definition of shapes?}  We should also request
...
\Alt{
\infer[Susp-Nat/Alt2]
  {\shape {\semenv(\tau)} = \exi \tvs {\deshaped \typ \tys} \\
...
\Alt{
  \infer[Susp-Ctx]
    {\Cshape \C \tv {\exi \tvs \sh} \\
      \semenv \th
         \C \where {\cexists \tvs \cunif \tva \sh \cand \cmatch \sh \Delta f}
    }
...
The shape of a variable is undefined and we write $\shape \t = \bot$ or
$\shape \t \Undefined$.  The shape of a nonvariable monotype $\t$ is always
defined, which we write $\shape \t \Defined$ and is unique up
$\alpha$-renaming. We may then write $\shape \t = \sh$.

A nonvariable monotype can be uniquely decomposed into a pair $\exi \tvs
\deshaped \tyz \tys$ such that $\t$ is equal to $\tyz \where{\tvs \is \tys}$ and
$\exi \tvs \tyz$ is the shape of $\t$.  We then write $\t = \exi \tvs
{\deshaped \tyz \tys}$.
