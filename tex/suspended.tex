...
$x$ to sets of ground types $\glam \subseteq \Ground$ (the set of ground
instances of the type scheme guessed for $\x$).
%
We write $\semenv\where{\tv \is \gt}$ and $\semenv\where{\x \is \glam}$ for
...
\begin{enumerate*}
  ...
  \item Nominal patterns $\cpatrcd \ct$, binding the name $\T$ of a nominal type
...
(\ie of the form $\tfor \tvs \t$ where $\ftv \t \subseteq \tvs$).

\Xdidier {Where are pattern variables bound? At each pattern I gues. Then
pattenrns need to be $\alpha$-convertible, so that we can avoid capture.
When do we extend $\semenv$ with pattern bindings? The matches rule defines
an extension of $\semenv$ but never uses it.}

\Xdidier {Letters $\ct$ and $\T$ are too close, too easy to be
confused---the difference may even be take for a typo!}
\begin{mathparfig}
  {fig:patterns-oml}
  {Patterns for \OML}
  \begin{bnfgrammar}
...
The constraint generation $\cinfer \e \t$ (and its helper
variants) are defined in \cref{fig:constraint-gen}.
