...
In this appendix, we give the full details of the well-foundedness of the semantics
of constraints, that is the satisfiability judgement $\semenv \th \c$.
...
\Hand     \vdashPf{\semenv}{\ctwo} {Premise}
	...
	\begin{llproof}
	  ...
	  \nsimplePf{\C\where{\cmatch \t \cbrs}} {By \cref{lem:match-tv-is-not-simple}}
...
\Hand	  \shapePf{\square\where\Ca}{\t}{\sh}{From premise}
	...
	\begin{llproof}
	  ...
	  \simplePf{\cerase {\Cc\where\Ca\where{\cunif \t \gt} \cand \c}}{Lemma XXX}
	  \vdashPf{\semenv}{\cerase {\Cc\where\Ca\where{\cunif \t \gt}}}{Lemma XXX}
...
\Hand 	  \shapePf{\parens{\Cc\where\Ca \cand \c}}{\t}{\sh}{Above}
	...
	\begin{llproof}
	  ...
	  \simplePf{\cerase {\cexists \tv \Cc\where\Ca\where{\cunif \t \gt}}}{Lemma XXX}
	  \vdashPf{\semenv\where{\tv \is \gtp}}{\cerase {\Cc\where\Ca\where{\cunif \t \gt}}}{Lemma XXX}
	  \eqPf{\shape \gt}{\sh}{$\implies$E on $\Cshape {\Cc\where\Ca} \t \sh$}
\Hand 	  \shapePf{\parens{\cexists \tv \Cc\where\Ca}}{\t}{\sh}{Above}
	...
	\begin{llproof}
	  ...
	  \simplePf{\cerase {\clet \x \tv {\Cc\where\Ca\where{\cunif \t \gt}} \c}}{Lemma XXX}
	  \vdashPf{\semenv}{\cexists \tv \cerase {\Cc\where\Ca\where{\cunif \t \gt}}}{Lemma XXX}
	  \simplePf{\cerase {\Cc\where\Ca\where{\cunif \t \gt}}}{Lemma XXX}
	  \vdashPf{\semenv\where{\tv \is \gtp}}{\cerase {\Cc\where\Ca\where{\cunif \t \gt}}}{Lemma XXX}
	  \eqPf{\shape \gt}{\sh}{$\implies$E on $\Cshape {\Cc\where\Ca} \t \sh$}
...
\begin{lemma}[Inversion of suspension]
  ...
  \begin{proof}
    We use canonicalization (Theorem XXX) to induct on $\semenv \Th
    \C\where{\cmatch \t \cbrs}$ instead of $\semenv \th \C\where{\cmatch \t
    \cbrs}$.

    This simplifies the proof, but introduces a circular dependency between Theorem
    XXX and Lemma XXX.
    %
    However, this does not compromise the well-foundedness of induction, as
    the application of Lemma XXX (via Corollary XXX) within the proof of
    Theorem XXX is restricted to strictly smaller constriaints.
	...
	\begin{llproof}
	  ...
	  \nsimplePf{\C\where{\cmatch \t \cbrs}}{Lemma XXX}
...
\Hand         \VdashPf{\semenv}{\C\where{\cmatched \t \sh \cbrs}}{Premise}
	  ...
	  \proofcase{$\C \neq \Cp$}
	    ...
	    \begin{llproof}
	      ...
	      \vdashPf{\semenvp}{\cerase {\Ctwo\where{\cunif \t \gtp, \ctrue}}}{Lemma XXX}
	      ...
	      \vdashPf{\semenvp}{\cerase {\Ctwo\where{\ctrue, \cunif \tp \gtp}}}{Lemma XXX}
...
\Hand 	  \vdashPf{\semenv\where{\tv \is \gt}}{\C\where{\cmatched \t \sh \cbrs}}{Lemma XXX}
	\end{llproof}
      \proofcase{$\impliedby$}
