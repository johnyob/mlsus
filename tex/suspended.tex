...
as \Rule{U-Decomp}.\Xgabriel{I find the syntax $\pshapp \tys[\ti]$
  hard to read, I would prefer something more discoverable like $\pshapp (\tys, \t_i, \tys')$. Besides, I wonder if we could use the notation $\sh[\tys]$ for shape applications, reminiscent of a standard context-with-holes notation.} Rules \Rule{U-True}, \Rule{U-False} simplify conjunctions.
Rule \Rule{U-Trivial} occurs when a multi-equation $\ueq$ is trivial, that is, when it is either empty
($\eset$) or contains only a single type ($\cunif \t \eset$).\Xgabriel{I don't see a formal definition of $\trivial \ueq$ in math notation; ideally it should be in a figure somewhere. We could possibly replace $\trivial \ueq$ with $|\ueq| \leq 1$, the notion of size/cardinality of a multi-equation being obvious.}
...
$\tvbb$, as $\tvbb$ is free. \TODO{describe how the solver should
determine determinability}.
