...
\cref{fig/types/wf}}
\Xalistair{I have removed well-formedness. It was getting in the way of proofs.}
...
types.
\Xalistair{We left patterns abstract since types were abstract. Perhaps
we could give the syntax of patterns here? My idea was that we left types /
patterns abstract and we could then present our approach as a `framework'.
But I appreciate that this makes the explainations harder. Although now
thinking about it, we introduce polytypes before talking about semi-explicit
first-class polymorphism. We also duplicate bits from \cref{sec/language}}
...
version demands that the shape of $\t$ must already be known in the
constraint: the premise $\t \in \Typesz$ enforces that $\t$ is not
a type variable.
