...
%% Missing, to be removed when fixed.
...
equirecursive types.} $\semenv$ maps each type variable $\tv$ to a ground
types $\gt \in \Ground$ (the type guessed for $\tv$), and each term variable
$x$ to sets of ground types $\gabs \subseteq \Ground$ (the set of ground
instances of the type scheme guessed for $\x$).
%
...
\cand \ctwo$ if it satisfies both $\cone$ and $\ctwo$. Satisfying $\cexists \tv \c$ requires guessing a witness $\gt$ for $\tv$. The universal constraint $\cfor \tv \c$ requires $\c$ to be satisfiable for any binding $\where{\tv \is \gt}$. A unification constraint $\t_1 = \t_2$ is satisfied when $\semenv(\t_1)$ and $\semenv(t_2)$ are equal.

The rule for $\clet \tv \cone \ctwo$ states that $\cone$ must satisfied under \emph{some} instantiation of its bound variable, and that $\ctwo$ must be satisfiable when $\x$ is bound $\cabs \tv \cone$, or rather its semantic interpretation as a set of ground types.
...
\begin{mathparfig}[t]%
  ...
  \begin{bnfgrammar}
    \entry[Semantic environments]{\semenv}{\emptyset \and \semenv\where{\tv \is \gt} \and \semenv\where{\x \is \gabs}
    }
  \end{bnfgrammar}
...
Note that there could be other choices to type some of these features;
as we discussed earlier, it would be possible to also use the return
type of a record projection to help disambiguate it. We believe that
such more complex strategies could be translated into suspended match
constraints as well, it is the choice of the type system designer.
...
The type system itself has good stability properties, discussed below.
...
\item [Application equitypability]
  ...
  $\eapp f \e$ and $\eappp {\mathsf{app}} f \e$ and $\eappp {\mathsf{rev\_app}} \e f$ are themselves
  equitypable where $\mathsf{app}$ and $\mathsf{rev\_app}$ are the application function $\efun f \efun \x \eapp f
...
compositionality.
%
Compositionality does not hold, since the context $\E$ may be used to ensure
...
may has several overloading sites may be duplicated and thus resolved
...
Subject-reduction, which is more general than inlining, is thus also broken
...
$\gt$ (Rule \Rule{Multi-Unif}). Multi-equations are considered equal modulo
permutation of their members.

\parcomment{The algorithm}
...
\\
  ...
  \infer[AppR]
    {\greg \tv {\semenv(\t)} \tvs \gts \in \semenv(\x)}
    {\semenv \th \capp \x \t}
\\
  \semenv(\cabsr \tv \tvs \c) \uad\eqdef\uad \set{\greg \tv \gt \tvs \gts \in \GroundRegion :
...
\emph{ground regions} $\greg \tv \gt \tvs \gts$ that satisfy $\c$. Regional
$\Let$-constraints strictly generalize ordinary constraint abstractions, as
captured by the equivalence:
\begin{mathpar}
...
In particular, \Rule{S-Let} rewrites the usual let constraints into region
form.
%
...
let-binders; and \Rule{S-Let-ConjLeft}, \Rule{S-Let-ConjRight} hoist
...
\Rule{S-Match-Var} applies when the scrutinee is a variable $\tv$ and the
...
\begin{mathparfig}
  ...
  {The syntax and semantics of partial applications.}
\begin{minipage}[t]{0.4\textwidth}
  \begin{bnfgrammar}
    \entry[Constraints]{\c}{
      \dots \and \cpapp \x \ren \ueqs
    } \\
    \entry[Region renamings]{\ren}{
      \tv := \tvb \and \ren\where{\tv := \tvb}
    }
  \end{bnfgrammar}
\end{minipage}
\hfill
...
\infer[Papp]
% Forgive my sins @Didier
  {\semenv\compose \ren \th \ueqs \implies
   \gregren {\semenv \compose \ren} {\tvbs} {\gts} \in \semenv(\x)}
...
solved application of $\capp \x {(\renroot \ren)}$. The renaming $\ren$ map
$\x$'s root and regional variables to fresh copies, and $\ueqs$ accumulates the
solved equations copied so far. This enables efficient, incremental
instantiation of constraint abstractions: solved parts are reused immediately,
while unresolved constraints (\ie suspended constraints) can be solved later,
further refining the abstraction and propagation additional equations to the
application sites.

\parcomment{Semantics of partial applications}

The semantics of partial applications (\Rule{Papp}) requires that if $\semenv
\compose \ren$ is a solution to the equations copied thus far $\ueqs$ under the
renaming $\ren$, then $\semenv \compose \ren$ must be a prefix of a ground
region satisfying the abstraction bound to $\x$.
%
Thus the domain of $\ren$ must be a subset of the regional variables of $\x$.
As a result, the regional variables $\tv, \tvs$ of a let constraint $\cletr \x
\tv \tvs \ca \cb$ are bound not only in the body of the abstraction $\ca$, but
also in the constraint $\cb$, where they may appear in partial applications of
$\x$ via renamings---and only there. Hence, they cannot appear in $\cb$ when
...
\item
  \Rule{S-Papp-Exists} allocates fresh existential variables at the
  call site that are mapped to uninstantiated variables within the
  abstraction.

\item \Rule{S-Papp-Unif} copies multi-equations from the abstraction to the call
  site, provided we have already introduced the renamings by \Rule
    {S-Papp-Exists} and we have not copied them previously. \\
\end{enumerate}
...
\begin{mathparfig}[htpb!]
  ...
  \rewrite[S-Let-AppR]
    {\C\where{\capp \x \t} \\
     \C(\x) = \cabsr \tv \tvs \c \\
     \tvc \disjoint \t}
    {\C\where{\cexists \tvc \cunif \tvc \t
			       \cand \cpapp \x {\tv \is \tvc} \eset }}

  \rewrite[S-Papp-Exists]
    {\C\where{\cpapp \x \ren \ueqs} \\
     \C(\x) = \cabsr \tv {\tvs, \tvb} \c \\
     \tvb, \tvbp \disjoint \ren, \ueqs, \tvc}
    {\C\where{\cexists \tvbp \cpapp \x {\ren \where {\tvb \is \tvbp}} \ueqs}}

  \rewrite[S-Papp-Unif]
    {\C\where{\cpapp \x \ren \ueqs}\\
     \C(\x) = \cabsr \tv \tvs {\c \cand \ueq}\\
     \dom \ren = \reg \tv \tvs \\
     \ueqs \nvDash \ueq}
  ...
  \rewrite[S-Exists-Lower]
    {\cletr \x \tv {\tvas, \tvbs} \c
     {\C\where{\overline{\cpapp \x \ren \ueqs}}} \\
...
have been copied---that is, when the copied equations entail the abstraction's
body. We ensure the solver has copied all the renamed equations $\ueq\where\ren$
to the call site with the side condition $\C \centails \ueq\where\ren$.
\Rule{S-Let-Solve} remove a $\Let$ constraint when
...
constraint context includes a partial application $\cpapp \x \ren \ueqs$
together with a multi-equation constraining the copy of $\tvp$ ($\ren(\tvp)$)
...
that our partial generalization and partial instantiations, but it
provides a different semantics where each instance can choose
...
dynamic overloading. We had to introduce the more complex suspended
match constraints to express features that require a unique resolution
of the constraint, typically for static overloading.
...
specification -- we would say that it is ``more principal''. On the
other hand, bidirectional typing is known to scale to powerful
type-systems such as fully-implicit predicative
...
\\
 ...
 \begin{array}{rcl@{}l@{\qquad} r}
   ...
   \c & ::= & \dots
              \mid \labenv(\elab/\ct) \leq \tone \to \ttwo
              \mid \labenv(\elab/\T) \leq \tone \to \ttwo
...
\clearpage % avoid mixing figures from above and from the Figures section below.
...
\\
\semenv(\cabsr \tv \tvs \c) \uad\eqdef\uad \set{\greg \tv \gt \tvs \gts \in \GroundRegion : \semenv\where{\tv \is \gt, \tvs \is \gts} \th \c}
\\
...
\newcommand{\Erule}[2]{\cerase {#1} &\eqdef& {#2}}
  ...
  \Erule{\cpapp \x \ren \ueqs}
      {\cpapp \x \ren \ueqs}\\
  \Erule{\cletr \x \tv \tvs \cone \ctwo}{\cletr \x \tv \tvs {\cerase \cone} {\cerase \ctwo}}
\end{tabular}
\end{mathparfig}
\FloatBarrier
...
\begin{mathparfig}
  ...
  \inferrule[Proj-I]
    {\eshape \E \e {\any \tvcs \Pi\iton \tvcs} \\
     \G \th \E\where{\exfield \e n j} : \ts}
    {\G \th \E\where{\efield \e j} : \ts}
  ...
  \inferrule [Poly-I]
    {\Eshape \E \e {{\any \tvcs \tpoly \tsp}} \\
     \G \th \E \where{\epoly[\exi \tvcs \tsp] \e} : \ts}
    {\G \th \E \where{\epoly \e} : \ts}
  ...
  \inferrule [Use-I]
    {\eshape \E  \e {\any \tvcs \tpoly \tsp} \\
     \G \th \E\where{\exinst \e \tvcs \tsp} : \ts}
    {\G \th \E\where{\einst \e} : \ts}
...
\eshape E \e \sh \Eqdef
  \forall \G, \ts, \gt, \uad
  \G \th \eerase {\E \where {\emagic {\eannot \e {} \gt }}} : \ts
      \wide\implies \shape \gt = \sh
\\[1ex]
\Eshape E \e \sh \Eqdef
  \forall \G, \ts, \gt, \uad
      \G \th \eerase {\E\where{\eannot {\emagic \e} {} \gt}} : \ts
      \wide\implies \shape \gt = \sh
\end{tabular}}
\\
\Eshape \Lab \elab \T \Wide\eqdef
   \forall \G, \ts, \gt , \uad
     \G \th \eerase {\Lab[\elannot {\elmagic \elab} {} \gt]} : \ts
	\implies \shape \gt= \any \tvcs {\tvcs \T}
\end{mathparfig}
\FloatBarrier
...
\begin{mathparfig}[htpb!]
  ...
  \inferrule[Simple-App]
    {\eone \simple \\ \etwo \simple}
    {\eapp \eone \etwo \simple}
  ...
  \inferrule[Simple-Rcd]
    ...
    {\erecord {\ela = \eone\; \ldots\; \el_n = \en}}
...
\judgbox{\eerase \e}
  {The erasure of $\e$.}
\begin{mathparfig}
  {fig:defn-term-erasure}
  {The erasure of implicit terms.}
\newcommand{\Erule}[2]{\eerase {#1} &\eqdef& {#2}}
  \begin{tabular}{RCL}
  \Erule{\x}{\x} \\
  \Erule{\efun \x \e}{\efun \x \eerase \e} \\
  \Erule{\eapp \eone \etwo}{\eapp {\eerase \eone} {\eerase \etwo}} \\
  \Erule{\eunit}{\eunit} \\
  \Erule{\elet \x \eone \etwo}{\elet \x {\eerase \eone} {\eerase \etwo}} \\
  \Erule{\eannot \e \tvs \t}{\eannot {\eerase \e} \tvs \t} \\
  \Erule{\etuple {\eone, \ldots, \en}}{\etuple {\eerase \eone, \ldots, \eerase \en}} \\
  ...
  \Erule{\erecord {\ela = \eone\; \ldots\; \el_n = \en}}{\erecord {\eerase \ela = \eerase \eone\; \ldots\; \eerase {\el_n} = \eerase \en}}\\
  \Erule{\efield \e \el}{\efield {\eerase \e} {\eerase \el}}\\
  \Erule{\elab/\T}{\elab/\T}\\
...
\judgbox{\G \thsimplesd \e : \t}
  {Under the typing context $\G$, the simple term $\e$ has the type $\t$.}
\begin{mathparfig}
  {fig:defn-simple-sd-typing}
  {Syntax-directed typing rules for \OML simple terms.}
  \inferrule[Var-SD]
    {x : \tfor \tvs \t \in \G}
  ...
  \inferrule[Let-SD]
    {\G \th \eone : \tone\\
     \tvs \disjoint \G \\
     \G, x : \tfor \tvs \tone \th \etwo : \ttwo}
    {\G \thsimplesd \elet x \eone \etwo : \ttwo}
\end{mathparfig}
\FloatBarrier
...
\begin{mathparfig}[htpb!]
  ...
  \rewrite[S-Papp-Solve]
    {\cletr \x \tv \tvs {\ueqs'} {\C\where{\cpapp \x \ren \ueqs \tvc}} \\
     ...
     \ueqs \vDash \ueqs' \\
     \x \disjoint \C}
    {\cletr \x \tv \tvs {\ueqs'} {\C\where{\ueqs\where{\ren\where{\tv \is \tvc}}}}}
...
\Hand 		  \VdashPf{\semenv}{\clet \x \tv \cone {\C\where{\cmatch \t \sh}}}{By \Rule{Can-Susp-Ctx}}
    ...
    \proofcasederivation
      ...
      {\semenv(\x) = \gabsr \tv \tvs \\ \dom \ren \subseteq \tvs \\
       \semenv\where{\tv \is \semenv(\t)} \th \ueqs\where\ren \implies \semenv(\t) \in \gabs}
      {\semenv \th \cpapp \x \ren \ueqs \t}
...
\begin{lemma}[Simple inversion of unifiers]
  ~
  \begin{itemize}
    \item If $\sub$ is a unifier of $\cunif \tone \ttwo$, then $\sub(\tone) = \sub(\ttwo)$.
...
\begin{lemma}
  For any term context $\E$, term $\e$, $\ctxinfer \E \tone \ttwo \where{\cinfer \e \tone} = \cinfer {\E\where{\e}} \ttwo$.
  \begin{proof}
...
\begin{lemma}[Simple context equivalence]
  For any two simple contexts $\Ca, \Cb$ and guard $P$. If
  ...
  \begin{proof}
    ...
    \begin{mathpar}
      \all{\C, \cs \simple} P(\cs) \implies \C\where\Ca\where\cs \equiv \C\where\Cb\where\cs
    \end{mathpar}
  ...
  \begin{proof}
	...
	\begin{itemize}
	    \proofcase{$\implies$}
	      ...
	      \TODO{This inversion is not entirely correct, requires another lemma}

	    \begin{llproof}
	      \simplePf{\Cpp, \Cp, \cs}{Premise ($n$ is 0)}
	      \vdashPf{\semenv}{\Cpp\where{\cletr \x \tv \tvs {\C\where\cs} {\Cp\where\ca}}}{Premise}
	      \vdashPf{\semenvp'}{\ca}{\cref{lem:simple-inversion}}
	      \eqPf{\semenvp'(\x)}{\semenvp(\cabsr \tv \tvs {\C\where\cs})}{\ditto}
	      \vdashPf{\semenvp'}{\cb}{$\dagger$}
\Hand	      \vdashPf{\semenv}{\Cpp\where{\cletr \x \tv \tvs {\C\where\cs} {\Cp\where\cb}}}{Simple reversion}
	    \end{llproof}
...
\Hand       \vdashPf{\semenv}{\cexists \tv \ca \cand \cb}{By \Rule{Exists}}
    ...
    \proofcase{\Rule{S-Let}, \Rule{S-Let-ExistsLeft}, \Rule{S-Let-ExistsRight}, \Rule{S-Let-ConjLet}, \Rule{S-Let-ConjRight}}

      \begin{llproof}
	Similar to the \Rule{S-Exists-Conj} case.
      \end{llproof}
...
\Hand 	    \vdashPf{\semenv}{\C\where{\cmatch \tv \cbrs}}{By \Rule{Susp-Ctx}}
    ...
    \proofcaserewrite
      {S-Let-App}
      {\cletr \x \tv \tvs \ca {\C\where{\capp \x \t}} \\ \tvc \disjoint \t \\ \x \disjoint \C}
      {\cletr \x \tv \tvs \ca {\C\where{\cexists \tvc \cunif \tvc \t \cand \cpapp \x \eset \eset \tvc}}}
	...
	\begin{llproof}
	  ...
	  \decolumnizePf
	  \sufficientPf{equivalence between}{\capp \x \t \text{ and } \cexists \tvc \cunif \tvc \t \cand \cpapp \x \eset \eset \tvc}{Lemma XXX}
	...
	\begin{itemize}
	  \proofcase{$\implies$}
	    ...
	    \begin{llproof}
	      ...
	      \inPf{\greg \tv {\semenvp(\t)} \tvs \gts}{\semenv(\x)}{Lemma XXX}
	      \vdashPf{\semenvp}{\ctrue\where\eset}{By \Rule{True}}
	      \eqPf{\semenv \compose \eset}{\eset}{}
	      \vdashPf{\semenv\where{\tvc \is \semenvp(\t)}}{\cpapp \x \eset \eset \t}{By \Rule{Papp}}
	      \vdashPf{\semenvp\where{\tvc \is \semenvp(\t)}}{\cunif \tvc \t}{By \Rule{Unif}}
\Hand	      \vdashPf{\semenvp}{\cexists \tvc {\cunif \tvc \t \cand \cpapp \x \eset \t}}{By \Rule{Exists} and \Rule{Conj}}
	    \end{llproof}
    ...
    \proofcaserewrite
      {S-Papp-Exists}
      {\cletr \x \tv {\tvs, \tvb} \c {\C\where{\cpapp \x \ren \ueqs \tvc}} \\ \tvb \notin \dom \ren \\ \tvb, \tvbp \disjoint \ren, \ueqs, \tvc \\ \x \disjoint \C}
      {\cletr \x \tv {\tvc, \tvb} \c {\C\where{\cexists \tvb \cpapp \x {\ren\where{\tvb \is \tvbp}} \ueqs \tvc}}}

	\begin{llproof}
	  \notinPf{\tvb}{\dom \ren}{Premise}
	  \disjointPf{\tvb, \tvbp}{\ren, \ueqs, \tvc}{Premise}
	  \disjointPf{\x}{\C}{Premise}
	  \decolumnizePf
	  \sufficientPf{equivalence between}{\cpapp \x \ren \ueqs \tvc \text{ and } \cexists \tvbp \cpapp \x {\ren\where{\tvb \is \tvbp}} \ueqs \tvc}{Lemma XXX}
	  \supposePf{\semenvp(\x) = \semenv(\cabsr \tv {\tvs, \tvb} \c)}{Premise}
	\end{llproof}
	\begin{itemize}
	  \proofcase{$\implies$}

	    \begin{llproof}
	      \vdashPf{\semenvp}{\cpapp \x \ren \ueqs \tvc}{Premise}
	      \vdashPf{\semenvp\where{\tv \is \semenvp(\tvc)}}{\ueqs\where\ren}{Lemma XXX}
	      \inPf{\greg[\semenvp \compose \ren] \tv {\semenvp(\tvc)} {\tvb, \tvbs} {\gt, \gts}}{\semenvp(\x)}{\ditto}
	      \eqPf{\dom \ren, \tvb, \tvbs}{\tvs}{$\semenvp(\x) = \semenv(\cabsr \tv {\tvs, \tvb} \c)$}
	      \eqPf{\ueqs\where{\ren\where{\tvb \is \tvbp}}}{\ueqs\where\ren}{$\tvb, \tvbp \disjoint \ueqs$}
	      \vdashPf{\semenvp\where{\tvbp \is \gt, \tv \is \semenvp(\tvc)}}{\ueqs\where{\ren\where{\tvb \is \tvbp}}}{Above}
	      \decolumnizePf
	      \inPf{\greg[\semenvp\where{\tvbp \is \gt} \compose \ren\where{\tvb \is \tvbp}] \tv {\semenvp(\tvc)} \tvbs \gts}{\semenvp(\x)}{Above}
	      \decolumnizePf
	      \vdashPf{\semenvp\where{\tvbp \is \gt}}{\cpapp \x {\ren\where{\tvb \is \tvbp}} \ueqs \tvc}{By \Rule{Papp}}
\Hand	      \vdashPf{\semenvp}{\cexists \tvbp \cpapp \x {\ren\where{\tvb \is \tvbp}} \ueqs \tvc}{By \Rule{Exists}}
	    \end{llproof}
	...
	\begin{llproof}
	  \eqPf{\dom \ren}{\tvs}{Premise}
	  \nvDashPf{\ueqs}{\ueq}{Premise}
	  \disjointPf{\x}{\C}{Premise}
	  \decolumnizePf
	  \sufficientPf{equivalence between}
	    {\cpapp \x \ren \ueqs \tvc \text{ and } \cpapp \x \ren {\ueqs, \ueq} \tvc \cand \ueq\where{\ren{\where{\tv \is \tvc}}}}
	    {Lemma XXX}
	  \supposePf{\semenvp(\x) = \semenv(\cabsr \tv {\tvs} \c \cand \ueq)}{Premise}
	\end{llproof}
	\begin{itemize}
	  \proofcase{$\implies$}
	    ...
	    \begin{llproof}
	      \vdashPf{\semenvp}{\cpapp \x \ren \ueqs \tvc}{Premise}
	      \vdashPf{\semenvp\where{\tv \is \semenvp(\tvc)}}{\ueqs\where\ren}{Lemma XXX}
	      \inPf{\greg[\semenvp \compose \ren] \tv {\semenv(\tvc)} {} {}}{\semenvp(\x)}{\ditto}
	      \vdashPf{\semenv\where{\tv \is \semenv(\tvc), \semenvp \compose \ren}}{\c \cand \ueq}{Definition of $\semenv(\cabsr \tv \tvs {\c \cand \ueq})$}
	      \vdashPf{\semenv\where{\tv \is \semenv(\tvc), \semenvp \compose \ren}}{\ueq}{Lemma XXX}
	      \vdashPf{\semenvp\where{\tv \is \semenv(\tvc), \semenvp \compose \ren}}{\ueq}{$\semenvp$ is an extension of $\semenv$}
	      \vdashPf{\semenvp\where{\tv \is \semenvp(\tvc)}}{\ueq\where{\ren}}{Lemma XXX}
	      \vdashPf{\semenvp}{\cpapp \x \ren {\ueqs, \ueq} \tvc}{By \Rule{Papp}}
	      \vdashPf{\semenvp}{\ueq\where{\ren\where{\tv \is \tvc}}}{Lemma XXX}
\Hand	      \vdashPf{\semenvp}{\cpapp \x \ren {\ueqs, \ueq} \tvc \cand \ueq\where{\ren\where{\tv \is \tvc}}}{By \Rule{Conj}}
	    \end{llproof}
	...
	\begin{llproof}
	  \eqPf{\dom \ren}{\tvs}{Premise}
	  \vDashPf{\ueqs}{\ueqs'}{Premise}
	  \disjointPf{\x}{\C}{Premise}
	  \decolumnizePf
	  \sufficientPf{equivalence between}
	    {\cpapp \x \ren \ueqs \tvc \text{ and } \ctrue}
	    {Lemma XXX}
	  \supposePf{\semenvp(\x) = \semenv(\cabsr \tv {\tvs} \ueqs')}{Premise}
	\end{llproof}
	\TODO{There are some issues with the semantics of partial applications that
	need fixing before we can complete this proof}
