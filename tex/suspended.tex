...
\Xgabriel{Each new language feature would need to introduce its own domain-specific constraint formers; for example to type nominal records you need to fetch field declarations from the declaration environment. I would rather leave this to the meta-level to keep the core language simple.}
...
sequence of free variables of $\t$ in order of appearance in $\t$. \Xgabriel{``Existentially'' suggests that shapes are constraints. As far as I can tell, thinking of $(\nu)$ as a $(\lambda)$ is more accurate than an $(\exists)$.}
...
independent of the choice of vairbales $\tvs$ for representing the shape. \Xgabriel{I would expect the standard notation $\sh(\tys)$.}
%
A type $\t$ has a decomposition $\deshaped \sh \tys$ iff $\t = \tys \shapp$;
we say that $\sh$ is \emph{a} shape of $\t$.  A decomposition is trivial is
the shape is $\bot$. \Xgabriel{It is obvious that $\sh(\tys)$ is uniquely determined in the paragraph above, but not completely obvious that the decomposition of $\tau$ as $\sh(\tys)$ is uniquely determined by $\sh$.}
...
\begin{mathpar}
  \forall \sh \in \Shapesz, \shp \in \Shapes, \uad
    \sh \preceq \shp \wedge
    \sh = \any \tvs \t \wedge
    \shp = \any \tvps \tp
  \implies
    \exi \tvs {f(\t)} \cequiv \exi \tvps f(\tp)
\end{mathpar}
\Xgabriel{I would call this \emph{uniformity} rather than parametricity. I believe that the definition is wrong: for example, it is not the case a-priori that $(\exists \tv. f(\tv~\tlist)) \equiv f(\tint \tlist)$. Instead we want to ask that if the substitution $\theta$ witnesses $\sh \leq \sh'$ then we have $f(\sh') \cequiv f(\sh)[\theta].$}

\Xgabriel{Many remarks in the rest of the text below read incorrectly if we consider that $\bot$ is a valid shape. Should we consider defining shapes in a way that actually rules $\bot$ out? Then the decomposition $\ty = \sh(\tys)$ becomes a partial relation, and/but I think this is fine?}
...
(\Rule{Susp-Nat}) that only applies when $\tau$ already has a shape \Xgabriel{Shouldn't we ask for a non-trivial shape in this rule?}:
\begin{mathpar}
  \infer[Susp-Use]
    {\t = \deshaped [\tvs] \typ \tys \\
