...
\Xdidier{I used \MLF, which allows inline polytypes.
Otherwise, I would use an example with nominal records instead of
...
in~\Cref{fig:constraint-syntax}.  Types are as usual type variables $\tv$,
the unit type $\tunit$, arrow types, structual tuples $\Pi {\ti}\iton$,
nominal types $\tys \T$, or polytypes $\tpoly \ts$ where type schemes $\ts$
are, as usual, of the form $\all \tvs \t$, since we treat them up to the
...
\Xdidier{I wanted to cut \emph{they are somewhat difficult
to understand} as I disagreed and but Alistair disagreed with the
cut:}\Xalistair{I still find them somewhat confusing, I propose we let
Gabriel break this stalemate}\Xdidier {So, instead, I propose the more
detailed explaination tha comes next:} they may be sometimes be unintuitive
as some information that has just been inferred must still be considered as
yet-unknown until its generalization.  They are also sensitive to the
placement of type annotation, an artifact of the fixed directionality of
generalization in
...
\cref{fig/types/wf}, is used to ensure the well-formedness of $\t$ under the
context $\G$.
