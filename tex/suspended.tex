...
types $\gt \in \Ground$\Xdidier[ (the type guessed for $\tv$)]{Not really,
this is the semantics, we do not guess anything in the semantics, which is
math, but take ground types to test/check validity of assertions. We should
cut.}, and each term variable $x$ to sets of ground types $\gabs \subseteq
\Ground$ (the set of ground instances of the type scheme guessed for $\x$).
%
...
\begin{mathline}
  \cmatches[\eqdef] \cpatwild {\pshapp \tys} \eset
\end{mathline}
...
%       {
% 	\cmatches \cpatwild {\pshapp[\tint]\cdot} \eset
% 	\\
% 	\infer*[Right=True]
% 	  { }
% 	  % ---------------------
...
\begin{mathpar}
  \cmatched \t \Sh {\parens{\cbranch \cpati \ci}}\iton \Wide\eqdef
    ...
    \eqno {\text{if}\quad \bigg\{\begin{tabular}[c]{L.}
      \cmatches \cpati {\shapp [\Sh] \tvs} \theta \\
...
\paragraph{\textsc{OutsideIn}} \Xalistair{volunteered to write this.}
