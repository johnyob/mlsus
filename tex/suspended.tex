...
the need for row polymorphic \Xgabriel[and any associated runtime
representation overhead]{A polymorphic numeric-field-accessor can be
  implemented efficiently as long as numeric fields have the same
  layout at all types. I would drop the remark which could put SML
  people on the defensive.}. The restriction is not reflected in the typing
...
Suppose we attempt to apply \Rule{Susp-Ctx} to the suspension on $\tva$ first.
...
for $\tva$ can be justified, and the application of \Rule{Susp-Ctx} fails.

Now consider instead applying \Rule{Susp-Ctx} to the suspension on $\tvb$
...
applying \Rule{Susp-Ctx}, now considering the context $\Cp \is \square \cand
...
satisfiable, then $\gt$ has the shape $\tbool$. Applying \Rule{Susp-Ctx} again
...
apply \Rule{Susp-Ctx} to the remaining match constraint on $\tva$. This
...
\subsection{Variants on static overloading---Variational types}
...
\subsection{Principal shapes}

\newcommand{\tc}{\mathprefix{c}}

\Cref {th/shapes/principal} states that any non variable type $\t$ has a
principal shape. We show that this is indeed the case.

When $\t$ is not a polytype, it has a toplevel constructor $c$ of fixed
arity $n$, which in our setting may be the nullary $\tunit$, the binary
arrow, an n-ary product, or a nominal type of some fixed arity. In all such
cases, the shape of $\t$ is $\any \tvcs \tc \tvcs$ where $\tvcs$ is a
sequence of $n$ distinct type variables, the arity of~$\tc$.
...
extended to reach a variable in $\tvcs$, in lexicographic order and $\tvcs$
be a sequence $(\tvci)\iton$ of distinct variables that do not appear in~$\t$.
%
Let $\tyz$ be $\t \where {\pi_i \is \tvci}$, \ie the term $\t$ where each
...
Let $\ti$ be $\t/\pi_i$ and $\tys$ be the sequence $(\ti)\iton$.
%
By construction, $\t$ is equal to $\shapp[\Sh] \tys$~\llabel 1.  Indeed, by
definition, $\shapp[\Sh] \tys$ is equal to $(\t\where {\pi_i \is \tvci}\iton)
\where {\tvci \is \ti}$ which is obviously equal to $\t$.

It remains to check that $\Sh$ is principal~\llabel 2.  Thus, assuming that
$\Sh'$ is another shape such that $\tpoly {\all\tvs\t}$ is
equal to $\shapp [\Shp] \typs$ for some $\typs$~\llabel H, we show $\Sh
\preceq \Shp$~\llabel C.
...
\end{enumerate*}
\relax
We may assume \Wlog that $\tvbs$ does not contain
variables, \ie that they all appear in $\tp$ and that they actually appear
in lexicographic order.
\relax
Then, the equality~\lref {H2} implies that $\t$, \ie $\t\where {\pi \is
\tvci} \where {\tvci \is \ti}$ is equal to
\relax
$(\tp \where {\tvbs\is \tvas}) \where {\tvcps \is \typs}$.
\relax
Since $\t'$ does not contain any variable in $\tvs$, all paths $\pi$ must
end in $\t' \where {\tvbs \is \tvas}$.
\relax
Therefore, $\t'$ and can be extended to the end of $\tp$, \ie so that $\tp$
is equal to $\tyz \where {\tvci \is
\tyip}\iton$ (and $\tyip$ is actually $\tp / \pi_i$).
\relax
We have $\any
\tvas \tyz \preceq \any \tvbs \tyz \where {\tvci \is \tyip}\iton$,
by Rule \Rule {Inst-Shape}, which is then nothing else but~\lref C.
