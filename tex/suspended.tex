...
\end{bnffig}
\Xalistair{There is an issue with the above figure, namely type varialbes
shouldn't have a $::=$. I tried fixing this, may fix later, alternatively,
Didier, if you could fix it, that would be great!}

% Syntax!
Building on this foundation, we adopt a constraint language that includes
both term and type variables. Its syntax is given in~\Cref{fig:constraint-syntax}.
%
...
That is $\expoly \e \ts$ can also be assigned the type scheme $\all \av
\tapoly \ts \av$. 
...
$\t$ that is an instance of $\ts$. If, by constrast, $\e$ hash the type
...
somewhat difficult to understand and sensitive to the placement of type
...
only in the position of the annotation, yet only the first is well-typed.
\begin{mathpar}
...
polytype instance, \OCaml completements \geninst-directionality with a limited form
...
\Xgabriel{It would be nice to be able to show some code examples and explain how they work in our
setting, informally -- or, sometimes, how the naive/simple elaboration is not enough.}
...
expressions, such as  \code{e}$_{10}$ are well-typed under our account but rejected
...
\begin{mathparfig}%
  ...
  \ctxindex \deriv \square \eqdef \deriv

  \deriv :: \infer
    {\semenv \th \cone \\
      \derivp :: \semenv \th \ctwo }
    {\semenv \th \cone \cand \ctwo}
  \implies
  \ctxindex \deriv {\cone \cand \C} \eqdef \ctxindex \derivp \C

  \deriv :: \infer
    {\derivp :: \semenv \th \cone \\
      \semenv \th \ctwo}
    {\semenv \th \cone \cand \ctwo}
  \implies
  \ctxindex \deriv \C \cand \ctwo \eqdef \ctxindex \derivp \C

  \deriv :: \infer
    {\derivp :: \semenv, \tv := \t \th \c}
    {\semenv \th \cexists \tv \c}
  \implies
  \ctxindex \deriv {\cexists \tv \C} \eqdef \ctxindex \derivp \C

  \deriv :: \infer
    {\derivp :: \semenv \th \cexists \tv \cone \\
      \semenv, x := \cabs \tv \cone \th \ctwo}
    {\semenv \th \cletin x {\cabs \tv \cone} \ctwo}
  \implies
  \ctxindex \deriv {\cletin x {\cabs \tv \C} \ctwo} \eqdef \ctxindex \derivp {\cexists \tv \C}

  \deriv :: \infer
    {\semenv \th \cexists \tv \cone \\
      \derivp :: \semenv, x := \cabs \tv \cone \th \ctwo}
    {\semenv \th \cletin x {\cabs \tv \cone} \ctwo}
  \implies
  \ctxindex \deriv {\cletin x {\cabs \tv \cone} \C} \eqdef \ctxindex \derivp \C

\end{mathparfig}
\Xalistair{I started sketching the semantics for indexing within a hole, yet
it is not entirely clear what the semantics should be for let-constraints. We
could consider multi-hole contexts (for each duplicate), we could consider the
satisfiability check $\cexists \tv \cone$. So far, I've gone for the latter}
\Xalistair{For backwards propagation, we could try picking
a single hole (either satisfiability or instance), thus some instance constraints
could propagate. But we'd need to existentially pick the hole of interest before
the universal quantifier the unicity predicate}

\subsubsection{Well-foundedness}\label{par:well-foundedness}

\Xdidier{Please write an example of infinite-depth constraint.}

\begin{program}
let rec loop 'a =
'a = int
&& match 'a with (fun _ -> exists 'b. loop 'b)
in
exists 'a. loop 'a
\end{program}
