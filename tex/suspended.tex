...
\Xalistair{I'm pretty sure if we re-ordered these types, we'd avoid
the annoying 'if you reorder the types then it fails' comments we have
in various subsequent examples}\Xdidier{I think so too, but
we would then need to change all the text. Also, these examples
should be the one reused in next section.}
\begin{program}[input]
type 'a one = {x : 'a}
...
only be a \code{int one}, and that \code{r.y} necessarily infers \code{two},
but field accesses such as \code{r.x} must be disambiguated. Consider for
example:
\begin{program}[input]
...
\code{r} must be \emph{known} to be either an instance of either
\ocaml{'a one} or \ocaml{two}. The definition \ocaml{e_1} is
clearly ambiguous, as there is no clue to the type of \ocaml{r}, and
...
\relax
By contrast, in both \ocaml{e_2} and \ocaml{e_3}, the only possible
type for \ocaml{r} is \ocaml {'a one}. Yet, there is a subtle distinction: Indeed, the
...
\Xalistair{I think this example misses the point a bit -- upon rereading it,
it not clear what we're trying to say here. I think the reader gets lost---
we're essentially showing pidirectional is weak since $\e_2$ and $\e_3$
are morally the same, but $\e_2$ works and $\e_3$ doesn't.}
\begin{program}[input]
...
\begin{program}[input]
let e_4 p r = if p then r.x else (r : two).x °\ocamlflags 20°
\end{program}
...
\Xalistair{This paragraph was in the previous section. I felt it was
more related to the benefit of omnidirectional inference as opposed
existing approaches. I've called it Illustrative examples, but feel free to rename
if you think of something better.}

Examples \code{e_2} to \code{e_4} are all typable with omnidirectional
...
\Xalistair{I think we could make it clearer that this trick with annotation
variables is \Geninst-directional inference. I don't think this is explicit
