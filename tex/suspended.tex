...
\begin{hparens}
\Xalistair{Shapes v3}
A shape $\sh$ is a \emph{partial type}:
\begin{mathpar}
  \begin{bnfgrammar}
    \entry[Shape]{\sh}{ \shq \tvcs \t }
  \end{bnfgrammar}
\end{mathpar}
where $\tvcs$ are bound within the type $\t$. Shapes must be closed.
We consider $\tvcs$ to be flexible within $\t$. The \emph{bottom} shape
...
For shape $\shq \tvcs \t$, we write $\tys \shapp[\shq \tvcs \t]$ for $\t \where {\tvcs \is \tys}$.
A type $\t$ has a decomposition $\angles {\sh, \tys}$ iff $\t = \tys \shapp$;
we say that $\sh$ is \emph{a} shape of $\t$. A decomposition is trivial is the shape is $\bot$.
...
\begin{theorem}
  For any type $\t$, if $\t$ is a non-variable type, then there is a non-trivial principal shape $\sh$
  for $\t$.
\end{theorem}
...
\end{mathpar}
We should also request that the meta-language for meta-functions is
terminating (i.e. doesn't have unbounded recursion).
\end{hparens}
\end{version}
