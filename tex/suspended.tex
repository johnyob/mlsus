...
\begin{program}[input]
let self f = f f      °\Ocamlcomment{\ocamlFlag {\MLF}1}°
\end{program}
...
whose type could easily be guessed. For instance, \MLF accepts
or rejects the following expression, depending on the position of the
annotation:\footnote{Green and red backgroun are to mean success or failure
of typechecking.}
\begin{program}[input]
...
its typechecking fails.\footnote {In fact, \OCaml uses a default
  ...
  the warning on default resolution.}  By contrast, the only possible type
  for \ocaml{r} is
  \ocaml {'a one} in both \ocaml{e_2} and \ocaml {e_3} -- . Still, there is
  a difference. Indeed, the type of~\ocaml{e_2} is considered to be
  unambiguous, while the type of
...
let e$_{10}$ r = r.x.x                           °\ocamlflags 11°
\end{program}
...
%
\OCaml also fails on \ocaml$e_8$. Omnidirectional inference
supports both examples, despite the fact that the disambiguation
information flows from an instance to a polymorphic definition, opposite to
the \Geninst-order. We call this \emph{back-propagation}.
...
we present on this work restricts disambiguation to the source type only and
rejects this example. But\Xdidier[we believe that]{Wht not the affirmative?}
...
\item
  Giving an adequate semantics for suspended constraints is also hard, as we
...
\item
  ...
  posponed to \cref {app:oml-calculus}.
...
well-typed expression $\e$ to be treated as if it had any type:
\begin{mathpar}
  \inferrule[Magic]
    {\G \th \e : \t}
    {\G \th \emagic \e : \tp}
...
\TODO{This lemma needs to be proven for susp-ctx closed guards. }
...
\Hand	    \vdashPf{\semenv}{\cpinst \inst \tv \tvca \cand \cunif \tvca \tvcb}{By \Rule{Conj}}
	...
	\begin{llproof}
	  \equivPf{\cfor \tvp \cexists {\tv, \tvs \setminus \tvp} \ueqs}{\ctrue}{Premise}
	  ...
	  \sufficientPf{equivalence between}
	    ...
	    {Lemma XXX}
	  \supposePf{\semenvp(\x) = \semenv(\cabsr \tv {\tvs} \ueqs \cand \c)}{Premise}
	\end{llproof}
	\begin{proofcases}
	  \proofcase{$\implies$}
