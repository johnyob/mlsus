...
\Xalistair{I prefer this definition of shape wrt the guarentees it gives
  about scoping. Yet, the constraint semantics are wrong. For instance, by permitting the
  metafunction to operate on types, it is possible that the two constraints
  $\cunif \tv \tint \tlist \cand \cmatch \tv {} f$ differs from $\cmatch {\tint \tlist} {} f$
  since the latter would permit the meta-function to `deeply' match on the type while the former
  doesn't. This is why shapes were open shallow types.}

\Xalistair{Also I'm more in agreement with you that perhaps $f$ shouldn't be a metafunction anymore
  but a list of constraint cases $\lambda \bar c$ where each case is a pattern $p \to \c$.
  However, I tried defining a notion of pattern and realised they'd have to be very dependent (e.g. in tuples, we'd
  need to be able to match on the size and index its subcomponents). Did you have a simple way to achieve this in
  mind?}
\end{version}
...
\paragraph{Shapes -- old definition}
...
\subsection*{Shapes -- new definition}

\blue

We define the shape of a type $\t$ as a generalization of the notion of type
constructor $\F$ to cope with polytypes. For that reason, we represent
shapes as existentially quantified shallow linear types $\exi \tvs \t$.
where $\tvs$ are the free variables of $\t$ and $\t$ is linear, \ie.  each
type variable $\tv$ in $\tvs$ occurs eactly only once in $\t$, and $\t$ is a
shallow type.  A shallow type that is not a polytype is one whose subterms
are all variables.  A shallow polytype $\tpoly {\all \tvs \t}$ is such that
all subterms that do not lead to a variable in $\tvs$ are variables.  The
shape of a variable is undefined (which we write $\shape \t = \bot$).
The shape of nonvarianle terms is is uniquely determined (up to
$\alpha$-renaming, indeed).
