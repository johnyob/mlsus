...
equalities in each branch.
Those type annotations may sometimes---but
...
to typecheck \code{self}, while neither of them is more general than the other,
...
\XDR [The following idea has been moved further down after we explain
      the basic examples.]
...
limitation of $\pi$-directional inference that type
...
\entry[Labels]{l}{
  \elab \and \elab / \T
\Xdidier{was commented out, but should be kept visible}
}
\\[1ex]
\entry[Types]{\t}{
   \tv \and
...
a multi-equation $\ueq$---we search for a decomposition $\C = \Ca\where{\ueq
\cand \Cb}$ where $\fvs \ueq$ is disjoint from the binders of $\Cb$.
...
\begin{mathparfig}[t]
  ...
  \rewrite[S-BackProp]
    ...
    {\tvp \in \reg \tv \tvs \\
     \cunif {\tvp\XDR{$\tvc$?}} {\cunif \t \ueq} \in \C\where\Cb \\
     \x \disjoint \bvs \Cb}
...
together with a multi-equation constraining the copy of $\tvp$
($\tvc$)\XDR{should be \emph{the copy $\tvc$ of $\tvp$}---if this is what
you mean. See comment in Rule \Rule{S-BackProp}}
...
has not been combinded with ML-style type inference yet, but this would be
...
remains unclear.  Should we fire all default rules of all suspended
