...
\TODO{We should explicitly point out that we use prefix notation for type constructors, except in
OCaml examples.}
...
\TODO{We are wondering about moving the rest of this section to an appendix.}
...
{\cunif \tva \tvb \cand \cmatch \tvb {\cbranch \cpatwild {\cunif \tvb \tvc}}}
...
\TODO{This should go to its own section before Discussion.}

\Xgabriel{I'm planning to compress this.}
...
Repeatedly generalzing a region after every update is expensive.  Instead,
we adopt a lazy approach: we track updates to the region using a
\emph{generalization tree}, marking regions as ``visited'' when they may
require re-generalization. Only when an instance is taken do we force
generalization of a region and all of its descendants---the descendants must
be fully generalized before generalizing the parent region, since
...
The typing Rule \Rule {Over-X} for explicit definitions is obvious
\begin{mathpar}
\infer [Over-X]{
    \exover \Mi \x : \t \in \G
}{% --------------------------
...
\infer [Over-I]{
    ...
    \Discriminates \Sh {\seq {\G(\Mi.\x)}}
}{% --------------------------------------
    \G \th \E \where {\eover \x} : \t
}
\end{mathpar}
The interesting rule is \Rule {Over-I} for (implicit) uses of overloaded
...
\Xdidier{\vskip 1ex Missing text here \vskip 1ex}
...
%% be adapted. In the current setting, a shape always discriminates amount the
%% different patterns of a match constraint $\cmatched
%% \t {\any \tvcs \tp} {\cbranch \cpats \cs}$.  This is not the case any longer
%% when shapes are non caonical.
...
\begin{mathpar}
  \cmatched \t \Sh {\parens{\cbranch \cpati \ci}}\iton \Wide\eqdef
    ...
    \eqno {\text{if}\quad \bigg\{\begin{tabular}[c]{L.}
      \cmatches \cpati \Sh \tvs \theta \\
      \discriminates {\Sh} {\parens\cpati\iton}
    \end{tabular}}
\end{mathpar}
Where patterns may now be non shallow terms.


%% \Xdidier{The end of this section is drafty}
%% \InputIfFileExists{overloading.tex}{}{}
...
\Xgabriel{I'm planning to re-read this.}
...
information only flows from the outside into the implication's conclusion. They
do not give a declarative semantics for delayed implication that their solver
preserves. Moreover, later work on \OutsideIn argues
\citep{Vytiniotis-Peyton-Jones-Schrijvers-Sulzmann/outsidein@jfp2011} -- rather
controversially -- that delayed implication constraints make local
let-generalization all but unmanagable, both in theory and implementation.
Their proposed fix is to abondon local let-generalization altogether. We
disagree, having solved the troubling interactions between let-generalization
and suspended constraints in this work.


\subsection{Principality tracking in \OCaml}
\Xgabriel{This is for me.}
\TODO{We discussion this in the introduction, let's point to that section again here.}
