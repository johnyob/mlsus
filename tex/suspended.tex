...
\begin{lemma}[Unification termination]
  ...
  \begin{proof}
	...
	\iw \tv &\eqdef& 0\\
	\iw {\shapp \tys} &\eqdef& \sw \sh + \sum\iton \iw \ti
      \end{tabular}
    \end{mathpar}
    We define the weight of a multi-equation as the sum of its
	...
	Hence $\uw {{\cunif {\pshapp \tvs} {\cunif {\pshapp \tvbs} \ueq}}} > \uw {\cunif {\pshapp \tvs} \ueq \cand \cunif \tvs \tvbs}$.
      ...
      \proofcaserewrite
	{U-Name}
	{\cunif {\pshapp \parens{\ta, \ldots, \ti, \ldots, \t_n}} \ueq \\ \tv \disjoint \tys, \ueq \\ \ti \notin \TyVars }
	{\cexists \tv {\cunif \tv \ti \cand \cunif {\pshapp \parens{\ta, \ldots, \ti, \ldots, \t_n}} \ueq}}

	We have
	\begin{mathpar}
	  \begin{tabular}{RCL}
	    \uw {\cunif {\pshapp {\parens{\ta, \ldots, \ti, \ldots, \t_n}}} \ueq} &=& \sw \sh + \sum_{j \neq i} \iw \tj + \iw \ti - 1 + \uw \ueq \\
	    \uw {\cexists \tv {\cunif \tv \ti \cand \cunif {\pshapp \parens{\ta, \ldots, \tv, \ldots, \t_n}} \ueq}}
	      &=& 1 + \tw \ti + \sw \sh + \sum_{j \neq i} \iw \tj - 1 + \uw \ueq
	  \end{tabular}
	\end{mathpar}
...
\termination
\begin{proof}
  The termination argument for the constraint solver follows similar lines to
  that of unification.
  ...
  Finally, the size weight (when defined) is preserved by all other rules, except the
  \Rule{S-Match*} rules, which may increase the weight. However, these rules are destructive
  (they eliminate a match constraint), and hence cannot be applied indefinitely.
  Thus, termination follows from a lexicographical ordering of the number of match
  constraints and the weight of constraints. THis order strictly decreases
  across all rules of interest and is preserved by all obviously destructive rules.
