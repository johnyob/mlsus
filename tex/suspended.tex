...
Macroyfied and easily reversible}
...
\begin{version}{\blue\False}%
\begin{hparens}
\Xdidier{Treatment of shapes:}
...
Below, I propose \Alt {alternative
formulations in red} followingf these rules. I would prefer to write $\t ?
...
is done by \Rule {Susp-Ctx}, which then allows \Rule {Susp-Use} to fire.
...
\Xalistair{I prefer this definition of shape wrt the guarentees it gives
  about scoping. Yet, the constraint semantics are wrong. For instance, by
  permitting the metafunction to operate on types, it is possible that the
  two constraints $\cunif \tv \tint \tlist \cand \cmatch \tv {} f$ differs
  from $\cmatch {\tint \tlist} {} f$ since the latter would permit the
  meta-function to `deeply' match on the type while the former doesn't. This
  is why shapes were open shallow types.}
\Xdidier
...
\Xalistair {Also I'm more in agreement with you that perhaps $f$ shouldn't
  be a metafunction anymore
  but a list of constraint cases $\lambda \bar c$ where each case is a
  pattern $p \to \c$.  However, I tried defining a notion of pattern and
  realised they'd have to be very dependent (e.g. in tuples, we'd need to be
  able to match on the size and index its subcomponents). Did you have a
  simple way to achieve this in mind?}
\end{hparens}
\end{version}
...
constructosr to cope with polytypes.  A shape $\sh$ is
an existentially quantified type $\any \tvs \t$ where $\tvs$ is exactly the
sequence of free variables of $\t$ in order of appearance in $\t$.  A shaped
is closed. It is treated up to $\alpha$-conversion.
%% We call $\t$ the \emph{structure}
%% of the shape, which by constrat is defined up to renaming.
We write $\bot$ for the partial shape $\any \tvc \tvc$.
...
\end{mathpar}
When writing $\sh \preceq \shp$, we say that $\sh$ is more general than $\shp$.

% Define decomposition

Given a shape $\shq \tvcs \t$, we write $\tys \shapp[\shq \tvcs \t]$ for $\t
\where {\tvs \is \tys}$. This is uniquely determined, since it is
independent of the choice of vairbales $\tvs$ for representing the shape.
...
defined).
%
Match functions operate on the principal decompositions, requiring the
...
\end{mathpar}
\vskip -\belowdisplayskip\vskip -\smallskipamount
\end{hparens}
\end{version}
...
(\Rule{Susp-Nat}) that only applies when $\tau$ already has a shape:
\begin{mathpar}
  \infer[Susp-Use]
    {\t = \deshaped [\tvs] \typ \tys \\
...
This rule is typically used in combinatino with \Rule {Susp-Use} which
can then be used in the prove of the right-hand side premise. 
...
\Alt{
  \infer[Susp-Use]
    {\deshaped \t \Defined \\
     \semenv_{\mid \Delta} \th f(\semenv \semenv \t)
    }
    {\semenv \vdash \cmatch \t \Delta f}
}
...
\begin{mathparfig}
  ...
  \bbrackets {\elab \leq \t \to \tp} \eqdef
    \cmatch \tp {\fvs \t} {(\lambda (\tvs \Fapp).~
    \mlet \ts {\D(\elab/\F)} \ts \leq \t \to \tvs \Fapp)}
\end{mathparfig}
