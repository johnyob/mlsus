...
\begin{mathpar}
  \infer[Sml-Proj]
    ...
    {\G \th \eproj \e j : \tj}
\end{mathpar}
\Xdidier {Beware! Overloading of meta-variable $\rho$}
...
principal shape, if it exists, and $\pshapp \tys$ for the recomposition of a
type where $\sh$ is a canonical principal shape. \Xdidier {Too many
...
\Xdidier {The notation $\shape \t \Defined$ is not longer defined before
beeing used. Actually, it should be equivalent to $\t \in \Typesz$, which is
a lighther more intuitive condition.}
...
and $\tvs$ are auxiliary existential variables.  Both $\tv$ and $\tvs$ are
bound in $\ca$ but not in $\cb$.  The order of $\tvs$ is immaterial, and
...
Rule \Rule{S-Let} rewrites the usual let constraints $\clet \x \tv \ca \cb$
into region form. Rule \Rule{S-Exists-Conj} lifts existentials across
conjunctions, while rules \Rule{S-Let-ExistsLeft} and \Rule{S-Let-ExistsRight}
lift existentials into or across let-binders. Rules \Rule{S-Let-ConjLeft} and
...
Rule \Rule{S-Susp-Ctx}. The rule selects a unique matching branch based on the shape
...
\end{program}
The generated constraint\footnote{Simplified for readability.} is of the form:
\begin{mathpar}
  \cexists \tv
