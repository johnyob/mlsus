...
\begin{program}[input]
let ex_4 p r = if p then r.x else (r : one).x °\ocamlflags 20°
\end{program}
...
In \ocaml{ex_6}, \ocaml{r} can only be of type \ocaml{one}. Indeed,
considering the second projection first, we should learn that
\ocaml{r} is of type \ocaml{one} and since it is $\lambda$-bound, this
...
\code{r} is forced to have the type \ocaml{one}; this requires
...
\ocaml{r} must be \ocaml{two}, as the \code{x} field of \code{one}
...
\Xalistair{Make sure this is correct}
...
constraints and partial generalization, in which we have reproduced the various
type-system features and examples presented in this work.  One closely follows
the constraint-based presentation described here\footnote{ The other prototype
is a direct implementation of type inference based on semi-unification. We
mention it here only it indicate that we have explored multiple implementation
strategies leading to the same results.}. It is public and open-source (link
omitted for anonymity). Its implementation is inspired by previous work such as
\Inferno~\citep {Pottier/inferno@icfp2014, Pottier/inferno@opam}.  It uses
state-of-the-art implementation techniques for efficiency, such as a Tarjan's
union-find data structure for unification \citep*{journals/jacm/Tarjan75} and
\emph{ranks} (or \emph{levels}) for efficient generalization. Let us discuss a
few salient points.
...
$\Let$-region at the same rank that are unrelated---neither an
...
the efficient integer comparisons of rank-based generalization.
...
from the partial generic (\textbf{PG}) to the instance. This enables the generic to
...
instantiation constraints track copies. In addition, the instance remains
...
\paragraph{Lazy generalization} Repeatedly generalzing a region after every
...
\Xalistair{Should the F in Future work be capitalized?}
...
backed by an efficient prototype implementation.
...
\emph{anyway} from our approach.  \Xalistair{I'm tired and tried making a pun with our title. Remove
if you think it doesn't work or you can think of a better one}
%
...
\citet*{Leijen-Ye/prefix@pldi2025}, which resolves overloaded identifiers via
disambiguation: the implicit form $\eover \x$, and an explicit counterpart
$\exover \M \x$, where $\M$ is a disambiguating module or namespace.
%
Although implemented in a prorotype, we have not yet identified a satisfactory
...
implicit abstraction $\eifun \t \e$ and resultion $\eimplicit \t$.

\paragraph{Higher-rank polymorphism}

In \cref{sec:introduction, sec:related-work}, we compared omnidirectional and
bidirectional type inference in the context of static overloading. While
overloading is nontrivial, it poses little challenge for the bidirectional
framework, making the comparison somewhat limited. Bidirectional typing is
best known for its scalability to more complex settings, such as higher-rank
polymorphism. We are therefore interested in extending our framework to support
higher-rank polymorphism, in the style of
...
\Xalistair{Didier I know you wanted to say something about \MLF, I'm
just not sure what you wanted to say.}
...
Our prototype includes an implementation of default rules---but design space
remains open. Should we fire all default rules of all suspended constraints
...
\citep*{Pottier-Remy/emlti}. Our prototype already handles such types,
...
can be recursive. While equality of shapes is syntactic in the acyclic setting,
this is no longer the case---but we do not believe this raises any issue.
