...
only be \code{int two}, and \code{r.y} necessarily infers \code{one} for \code{r}.
But field accesses such as \code{r.x} are ambiguous unless the type of \code{r} is
\emph{known}. Consider:
\begin{program}[input]
let ex_1 r = r.x                         °\ocamlflags 11°
let ex_2 = let r = {x = 1} in r.x        °\ocamlflags 00°
let ex_3 = (fun r -> r.x) {x = 1}        °\ocamlflags 10°
\end{program}
...
the expression \code{r.x} can only refer to the field from type \code{'a two}.
...
functions:
\begin{program}[input]
let ex_3_2 = (fun r -> r.x) @@ {x = 1}   °\ocamlflags 10°
let ex_3_3 = {x = 1} |> (fun r -> r.x)   °\ocamlflags 10°
\end{program}
...
\begin{program}[input]
let ex_4 p r = if p then r.x else (r : two).x °\ocamlflags 20°
\end{program}
...
semi-explicit first-clas polymorphism, and used in \MLF. It has since been adopted in
\OCaml for resolving overloading of record fields and variant constructors in
\OCaml, and more generally whenever the type-checker disambiguates on type information.
...
let ex_8 = let getx r = r.x in getx {x = 1}      °\ocamlflags 10°
...
accepts \ocaml{ex_6} with a warning, and fails if we reverse the order of the
type declarations.}, \OML accepts the first three.

In \ocaml{ex_6}, \ocaml{r} can only be of type \ocaml{two}. Indeed,
...
records that we present on this work restrict disambiguation to the
source type only and rejects this example. But other typing rules
...
ill-typed. This mechanism precisely realises \geninst-directionality: it is precisely
the polymorphism of $\av$ that ensures that the polytype is indeed \emph{known}
and not being inferred.
...
\Xalistair{Is it used for polymorphic nominal records? I was
under the impression we didn't need \code{Tpoly} for this.}
\Xgabriel{(my previous answer was about something else (generalizability as principality checking), I was not in the right context in the paper.)}
...
\begin{program}[input]
let ex_1_1 = {x = 42; y = 13} °\ocamlflags 00°
\end{program}
...
\begin{program}[input]
let ex_7 r = let x (* infers ['a two] *) = r.x in x + r.y °\ocamlflags 10°
\end{program}
Here, \OCaml defaults to \code{'a two} for \code{r} when typing \code{r.x},
but then fails to type \code{r.y}, as this default choice is fixed---even
thought \code{one} would have satisfied both projections.
...
rules themselves, it is specified in terms of the
...
\paragraph{Comparison to \SML} Our understanding is that \SML
type-checkers implement row-polymorphic records under the hood, but
they never generalize row variables and fail if a declaration contains
an undetermined row variable. This is a nice approach and we
conjecture that it accepts the same programs as our implementation of
overloaded tuples using suspended constraints. On the other hand, it
only works for structural types for which we know of a row-based
presentation enjoying good type-inference properties. It cannot be
applied to disambiguate between nominal records or variants that share
field or constructor names, especially when some field projections or
data constructors need non-uniform typing rules, such polymorphic
fields or GADT constructors.
...
\Xdidier{The last sentence repeats what is said on
  line 170}\Xgabriel{I would prefer to keep the repetition, because otherwise we mention the downsides here without the benefit.}
\Xalistair{Agree with Gabriel here.}
...
unification.
\Xdidier{We never clearly define unification on shapes.}
\Xalistair{Ah, a fatality of cutting. Perhaps
we can add one line in section 5.1 to mention that
we operate on shapes directly in unification.
This sentence is here to motivate why shapes are useful. }
...
defined by rule \Rule{Inst-Shape}.
%
...
\parcomment {Poltypes are constructors}
...
\begin{mathpar}
  \cexists \tv
    \clet x \tvb
      {(\cmatch \tva \dots)}
      {\cinst x \tint \cand \cunif \tv {\mathsf{one}}}
\end{mathpar}
...
\ocaml{'a two}. The projection \code{r.x} would be
forbidden if \code{r} had a variable type, so
\code{'a two} is the unique solution. We call this flow of
...
system. Surprisingly, identifying an appropriate declarative type
...
overloaded tuple projections \ala \SML. We can ask the user to provide the
...
\end{mathpar}
\end{wraphbox}
On the other hand, the natural typing rule for the fragile construct
$\efield e j$ breaks principality:
%
The term $\efield \e j$ admits many typings, as any tuple of at least size
$j$ satisfies the premise.
...
\hfil
\end{wraphbox}
This is the exact same issue we had with the naive semantics of suspended
...
construct into the robust, explicit construct:

We have to specify the condition $\eshape \E \e {\any \tvcs \Pi\iton
\tvcs}$, which is the counterpart of our unicity condition $\Cshape \C \t
\sh$ in the type system. We formally introduce it in the rest of this
section; in fact the different fragile features need slightly different
formulations of the unicity condition.
...
\item
  ...
  boxing $\epoly \e$ and $\expoly \e \tvs \ts$, and implicit and explicit
  unboxing $\einst \e$ and $\exinst \e \tvs \ts$.

\end{enumerate}
We use the metavariables $\e^i$ or $\el^i$ to range over the
fragile/implicit constructions, and $\e^x$ or $\el^x$ to range over their
explicit counterpart.

\subsection{Unicity conditions in typing rules}

Our typing rules for implicit constructs are inspired by the semantics: a
contextual rule with a unicity condition, and an elaboration into an
explicit rule. Our unicity conditions in typing rules request that the
annotating shape is fully determined from the context, and from the type of
either the context hole or the expression.
...
\end{mathpar}
\end{wraphbox}
In order to define the unicity conditions, we introduce a \emph{typed hole}
construct $\emagic \e$ that allows any well-typed expression $\e$ to be
treated as if it had any type:
%
...
We can now state that a shape is uniquely determined by the type of a
context or an expression:
\begin{mathpar}
\def \Eqdef {&\eqdef&}
{\begin{tabular}{RCL}
\eshape E \e \sh \Eqdef
...
the typing rules for implicit fragile constructs which use unicity
...
variables. The constraint generation for implicit constructs use
...
: \t$ and $\G \th \ez : \ts$, we do not necessarily have $\G \th \elet \x
...
specific to our support for \emph{back-propagation}: the $\Let$-form will
...
Rule \Rule{S-Match-Type} solves suspended match constraints whose scrutinee
is a non-variable type $\t$ by rewriting them using the sugar $\cmatched \t
{\shape \t}
...
Rule \Rule{S-Match-Var} applies when the scrutinee is a variable $\tv$ and
...
\begin{program}[input]
  let e_7 r = let x = r.x in x + r.y °\ocamlflags30°
\end{program}
...
\begin{minipage}[b]{0.5\hsize}
  ...
  \begin{bnfgrammar}
    ...
    \entry[\hspace {-2em} Semantic environments]{\semenv}{
      \dots
...
Rule \Rule{S-Let-Solve} remove a $\Let$ constraint when the bound term
variable is
...
Finally, Rule \Rule{S-BackProp} expresses \emph{backpropagation}, previously
...
\Xalistair{We should drop this if we cannot fit the two rules of interest in
the main paper.}\Xdidier{what is \emph{this}? paragraph above or below?}

\paragraph{Universal quantification}

Universally quantified constraints are solved using \emph{unification
  under a mixed prefix}. Our rules (in \cref{app/rules/unif}) are entirely
standard \citep*{Pottier-Remy/emlti} and are rather analogous to that
of $\Let$ constraints. There are two failure conditions:
\Rule{S-All-Escape} detects that $\cfor \tvs \cexists \tvbs \c$ is
false if a rigid variable $\tv$ is indirectly dominated by a free
variable $\delta$.  If so, the value of $\tv$ is determined by that of
$\delta$---but, a universally quantified variable ranges over all
values, resulting in a contradiction; \Rule{S-All-Rigid} occurs when
$\tv$ is equated with a non-variable type.  This similarly partially
determines the shape of $\tv$, contradicting $\tv$'s universality.
  ...
  \begin{enumerate}[(\roman*)]
    ...
    \item $\c$ is stuck, that is:
      ...
      \item
        ...
        $\x \disjoint \hat\C$ and $\insts {\hat\C} \inst = \eset$;
      \item
        for every match constraint $\hat\C\where{\cmatch \tv \cbrs}$ in
        $\c$, $\Cshape {\hat\C} \tv$ does not hold.
...
We have two working prototypes of a core \ML language with suspended match
constraints and partial generalization, in which we have reproduced the
various type-system features and examples presented in this work.  One is a
direct implementation of type inference based on semi-unification. It is
experimental and not released yet.  The other one, which we describe below,
is closely following the constraint-based presentation of this work.  It is
public and open-source (link omitted for anonymity).
Its implementation is inspired by previous work such as
...
Union-Find data structure and the use of \emph{ranks} (or \emph{levels}) for
efficient generalization. Let us discuss a few salient points.
...
type. At this point the wait list is flushed: the suspended constraints are
...
variable scope to the current region being inferred. Unification maintains
an integer \emph{rank} (or \emph{level}) for each variable, that index a
region in the stack.
%
...
the efficient integer comparisons.
...
\begin{itemize}
  ...
  \item[PG] Variables that are generalizable, but may be
    updated in the future. \\
    \emph{Variables mentioned by suspended match constraint or partial
    applications.}

  \item[PI] Variables that were previously partially generalized
    but have since been updated.  \\
    \emph{Awaiting re-generalization. Introduced by unification of partial
    generics.}
\end{itemize}
...
already been used in some dialect of \ML, with specific typing rules we
propose for each feature. Suspended match constraints is a framework that
could be used to encode typing rules for other fragile features.

Our nomimal records use a restricted form of overloading.  We have also
experimented with a more general form of overloading in one of our
prototypes. Although, we have not formalized them yet, we believe they fit
well in our formalism. One possibility is to extend the framework we
presented to to support patterns on arbitrary shapes (and not restricted to
shallow shapes), as long as none is a prefix of another.  An alternative
would be to encode matching on arbitrary shapes as a cascade of matching on
shallow shapes.  This is left for future work.


\begin{version}{}
\citet*{Leijen-Ye/prefix@pldi2025} propose a general mechanism for
overloading identifiers, with an implicit form $\eover \x$ and an explicit
form $\exover \M \x$ prefixed by a disambiguation name $\M$. We can extend
our approach to typing rules for this construct:
\begin{mathpar}
\infer [Over-X]{
    \exover \Mi \x : \t \in \G
}{% --------------------------
...
\end{mathpar}
The third premise of \Rule {Over-I} asks that there is a unique explicit
name $\exover \M \x$ that instantiates $\Sh$ in the context. The framework
we presented for \OML does not immediately allow to implement these rules,
as it only uses patterns that discriminate on minimal (shallow) shapes, but
it can be extended to support patterns on arbitrary shapes (as long as none
...
lexical scope. \SML has an overloading mechanism for numeric literals
that provides default types for each literal form~\citep*[Section 5.8]{rossberg-hamlet}.
...
generalizable. We compared their approach to ours in
\cref{sec/constraints/polytypes}.

\paragraph{Qualified types}

Qualified types~\citep*{jones-qualified-types}, most well-known via their
usage in Haskell type-classes, are related to our suspended match
...
pairs are delayed until at least one side becomes rigid. The contributions
of our work are the combination with implicit generalization, which is
typically absent from those systems, and the design of declarative semantics
to establish principality.
...
preserve the main metatheoretical properties, as in~\citet*
...
I am not even sure we need to mention~\citet*
...
\TODO{@Didier are you able to fix the odd spacing going on here? I'm at a loss of
      why mathpar is adding this space}\Xdidier{Where exactly?}
...
\begin{judgboxmathpar}
  ...
  \rewrite[S-Inst-Mono]
    {\cletr \x \tv \tvs \c {\C\where{\cpapp \x \tvb \tvc \inst}}}
    {\tvb \disjoint \tv, \tvs \\ \x, \tvb \disjoint \bvs \C}
    {\cletr \x \tv \tvs \c {\C\where{\cunif \tvb \tvc}}}
...
\begin{lemma}[Simple let equivalence]
  ...
  \begin{proof}
	...
	\begin{llproof}
	  ...
	  \vdashPf{\semenv}{\cexists {\tv, \tvs} \C\where{\cs}}{\cref{lem:simple-inversion}}
...
\begin{lemma}
  ...
  \begin{proof}~
    ...
    \begin{llproof}
      ...
      \eqPf{\gt}{\semenvb(\tv)}{\cref{lem:simple-inversion}}
      \continueeqPf{\semenva(\tv)}{$\tv \disjoint \bvs \Cb$}
      \continueeqPf{\shapp[\shape \t] {\semenva(\tys)}}{\cref{lem:simple-inversion}}
	...
	\begin{proofcases}
	  \proofcase{$\implies$}
	    ...
	    \begin{llproof}
	      ...
	      \inPf{\greg \tv {\semenva}}{\semenv(\x)}{\cref{lem:simple-inversion}}
	...
	\begin{proofcases}
	  \proofcase{$\implies$}
	    ...
	    \begin{llproof}
	      ...
	      \eqPf{\semenvp(\tvc)}{\semenv(\inst)(\tvp)}{\cref{lem:simple-inversion}}
	      \continueeqPf{\semenva(\tvp)}{Above}
	      \vdashPf{\semenva}{\cp \cand \cunif \tvp {\cunif {\shapp \tvbs} \ueq}}{Above}
	      \vdashPf{\semenva}{\cunif \tvp {\cunif {\shapp \tvbs} \ueq}}{\cref{lem:simple-inversion}}
...
\Hand 	      \vdashPf{\semenvp}{\cexists {\tvbs'} \cunif \tvc {\shapp \tvbs'} \cand \cpapp \x {\tvbs} {\tvbs'} \inst}{By \Rule{Exists} and \Rule{Conj}}
	...
	\proofcase{$\implies$}
	  ...
	  \begin{llproof}
	    ...
	    \vdashPf{\semenv}{\cpinst \inst \tv \tvca}{\cref{lem:simple-inversion}}
...
\Hand		  \vdashPf{\semenvp}{\cpapp \x \tvp \tvc \inst}{By \Rule{Partial-Inst}}
		...
		\begin{llproof}
		  ...
		  \vdashPf{\semenva}{\ueqs}{\cref{lem:simple-inversion}}
...
\Hand		  \commentPf{So this case degenerates to the former case.}{}
	...
	\begin{proofcases}
	  ...
	  \begin{llproof}
	    ...
	    \eqPf{\semenvp(\inst)}{\semenva}{\ditto}
	    \eqPf{\semenvp(\tvc)}{\semenva(\tvb)}{\cref{lem:simple-inversion}}
...
\Hand	    \vdashPf{\semenv}{\cletr \x \tv \tvs \ueqs \c}{By \Rule{LetR}}
  ...
  \proofcaserewrite{S-Exists-Lower}
	...
	\vdashPf{\semenv}{\cexists {\tv, \tvas, \tvbs} \ca}{\cref{lem:simple-inversion}}
...
\begin{proof}
  ...
  \begin{proofcases}
	...
	\begin{proofcases}
	  ...
	  \proofcase{$\hat\C\where{\cpapp \x \tvp \tvc \inst}$} We have $\x \disjoint \bvs {\hat\C}$
	    \begin{itemize}
...
Case \Rule {U-Name} is indeed broken (it is an equality), but we should find
...
Case \Rule {Decomp} is correct modulo a typo.
...
\end{version}
\Xgabriel{The argument on ``deconstruction'' and the handling of discharging rules are orthogonal. The weights on partial instantiations given by Alistair are not preserved by discharging rules, so the argument using those weights must necessarily be restricted to discharge-free sequences. My prose explains why we can do this restriction, in a more modular way than building a complex lexicographic ordering at the end of the proof.}

\termination
\begin{proof}
  The ``discharge'' rules \Rule{S-Match-Type}, \Rule{S-Match-Var} and
  \Rule{S-BackProp} discharge suspended constraints, and they can make
  ...
  \t$ decreases strictly on \Rule{S-Let-AppR} when a partial instantiation
  starts, and is preserved by other non-discharge rules. The rule
  \Rule{S-Let-AppR} can thus only occur finitely many time in
  non-discharging sequences, and it suffices to prove that all rewriting
  sequences that are non-discharging and are not \Rule{S-Let-AppR} are
  finite.
  ...
  unification rules) must be applicable.\Xgabriel{This sounds fishy. A
  constraint could a priori have a finite reduction sequence to $\cfalse$,
  and \emph{also} an infinite sequence of partial instantiation rules.}
  \Xalistair{Agreed. This is something I realised after coming up with
  semantic definition of height. I guess my point is in the solver, before
  partially instantiating, we would check if the region is not obviously false.

  My argument is along the lines of: if $\cerase \c$ is unsatisfiable, the
  solver can eagerly detect this (EMLTI style) \emph{before} instantiating.
  We could just add the condition that $\cerase \c$ is satisfiable (removing
  $\neg \cyclic \c$ (which follows from this).
  }
...
Let $\e$ be an arbitrary closed well-typed term; that is, there exists a type
  ...
  (specifically under the unifier $\cunif \tv \t$). By \cref{thm:correctness}, there
