...
\begin{definition}
  ...
  context $\C\where{\square}$, written $\Cshape \C \tv \sh$, iff for all assignments
  $\semenv$ and ground types $\t$, then $\semenv \vdash \C\where{\cunif \tv \t}$
...
that the conclusion of \Rule{Susp-Ctx} $\C\where{\cmatch \tv \cbrs}$ is structurally
larger than the negatively-occurring judgement $\C\where{\cunif \tv \t}$.
...
\begin{mathpar}
  \rewrite[S-BackProp]
    {\C\where{\cletr \x \tv \tvs {\Ca[\cmatch \tvp \cbrs}} {\Cb[\cpapp \x \ren \ueqs \tvc]}] \\
     \tvp \in \dom \ren \\
     \C\where{\Cb} \th \theta(\tv) = \t \\
     \shape \t = \any \tvcs \tp}
