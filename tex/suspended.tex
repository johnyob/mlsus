...
\semenv(\cabsr \tv \tvs \c) &\eqdef& \set{\greg \tv {\semenv\where{\tv \is \gt, \tvs \is \gts}} \in \GroundRegion :
...
Universally quantified constraints are solved using \emph{unification
  under a mixed prefix}. Our rules (in \cref{TODO}) are entirely
...
\begin{restatable}[Progress]{theorem}{progress}
  ...
  \begin{enumerate}[(\roman*)]
    ...
    \item $\c$ is stuck: either $\cfalse$; $\hat\C\where{\capp \x \t}$ where $\x \disjoint \hat\C$; $\hat\C\where{\cpapp \x \tv \tvc \inst}$
      where $\x \disjoint \hat\C$ and $\insts {\hat\C} \inst = \eset$; for every match constraint $\hat\C\where{\cmatch \tv \cbrs}$ in $\c$,
      $\nCshape {\hat\C} \tv$.
      Here $\hat\C$ is a \emph{solved} context \ie no other rewrites can be applied.
