...
\paragraph{Backpropagation} The interaction that froces
partial application to be useful\footnote{We could in theory re-order
instantiations in any way, delaying until the abstraction is fully solved} is
...
\begin{align*}
  ...
  &\clet f \delta {\cexists {\tvb, \tvc}
    \cunif \delta {\tvb \to \tvc}
    \cand \cmatch \tvb {(\cbranch {\wild \Tapp[t]} {\Omega(y / t) \leq \gamma \to \beta})}} \\
  &\hspace{-1mm}\capp f {(\mathsf{three} \to \tv)}
\end{align*}
...
predicate $\Cshape \C \tv \sh$ ensures that $\tvb$'s shape is \code{three}
provided the context $\C$ include the application constraint.
\begin{mathpar}
  \rewrite[S-BackProp]
    {\C[\cletr \x \tv \tvs {\Ca[\cmatch \tvp \cbrs]} {\Cb[\cpapp \x \ren \ueqs \tvc]}] \\
     \tvp \in \dom \ren \\
     \C[\Cb] \th \theta(\tv) = \t \\
     \shape \t = \any \tvcs \tp}
