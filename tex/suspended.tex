...
I think I prefer the deconstructing intuition of Alistair, which could be made
by defining a strict ordering in which constraints are deconstructed. For
example, it is always progress to
\begin{itemize}

\item remove a match constraint (as we never dupplicate them)
\item remove an $\x \t$ constraint, etc.
\item reduce the size of a let body.
\item remove part of a let body that does not contain an $x$
\item move an $\exists \tv$ outside of anything
\item \ldots
