...
semi-explicit first-clas polymorphism, and used in \MLF~\citep*
{LeBotlan-Remy/recasting-mlf}. It has since been adopted in
...
\Xgabriel{We don't need Tpoly, only to check via levels that the type is generalizable.}
\Xalistair{Generalizable, as in the polymorphic variables of the
record field $\tvs$ are universally quantified? If so, we don't
need semi-explicit first-class polymorphism for this, a simple $\cfor \tvs \c$
constraint will do. If you mean that the record type must be generalizable
for disambiguation, then this is static overloading not semi-explicit
first-class poly}
...
only works for features for which we know of a structural-type
presentation enjoying good type-inference properties. It would not
work when disambiguating nominal record fields that may have
a polymorphic type (using polytypes as we present in this paper), or
variant constructors that may be GADTs.
...
This semantics rule is nicely declarative, but unfortunately accepts to too
many constraints. Consider for example: $\cexists \tv \cmatch \tv {\cbranch
\cpatwild {\cunif \tv \tint}}$, which is satisfiable under this natural semantics:
\begin{mathpar}
...
To rule out guessing, we instead adopt a \emph{contextual} semantics: a match
constraint is satisfiable only if the shape of the type is determined by the
surrounding context. The corresponding rule for suspended constraints is the
...
\begin{definition}[Erasure]
  ...
  obtained by suspended match constraints in $\c$ with $\ctrue$.
\end{definition}
...
\begin{definition}[Unicity]
  We define the unicity condition $\Cshape \C t \sh$, which states that $\t$ has a unique canonical shape $\sh$ within the context $\C$.
  \begin{mathpar}
...
holds trivially for $\sh = \shape \t$. Likewise, if $\C$ is unsatisfiable,
...
In the first example, we apply the contextual rule with the context $\C \is
  \cexists \tv \cunif \tv \tint \cand \square$. Any solution $\semenv$ of this context
   necessarily satisfies $\cunif \tv \tint$, so we have $\Cshape \C
  \tv \tint$ and the suspended constraint can be discharged.

In constrast, the second example has no contextual information around the
  suspended constraint: $\C \is \square$. So any solution $\semenv$ satisfies
  it, allowing $\semenv(\tv)$ to have an arbitrary shape (\eg $\tint$, $\tbool$,
...
\square \cand \cunif \tva \tint$. Its erasure is $\cerase \C = \ctrue \cand \square \cand \cunif \tva \tint$.
  In this constraint $\tvb$ is unconstrained, so for example $\cerase {{\C\where{\cunif \tvb \tint}}}$ and $\cerase {\C\where{\cunif \tvb \tbool}}$ are both satisfiable:
  unicity does not hold and \Rule{Susp-Ctx} cannot be applied.
...
\cand \cunif \tva \tint$. Its erasure is $\cerase \C =
\square \cand \ctrue \cand \cunif \tva \tint$. Since $\tva$ is unified with
$\tint$ in the erasure, we have $\Cshape \C \tva \tint$.
%
We may now discharge the match on $\tva$, rewriting it as $(\cmatched \tva \tint {\cbranch \cpatwild {\cunif \tvb \tbool}})$---that
is, $(\tva = \tint \cand \cunif \tvb \tbool)$. Substituting back, we are left to satisfy the constraint $\C \where {\tva = \tint \cand \cunif \tvb \tbool}$, that is,
...
The unicity condition now holds, as the erasure of the context includes the
...
$\tva$ first. We must show $\Cshape \C \tva \tint$ where $\C \is  \square
\cand \cmatch \tvb {\cbranch \cpatwild {\cunif \tva \tint}}$ But the erasure
$\cerase \C = \square \cand \ctrue$ imposes no constraint on $\tva$, so
unicity fails, and \Rule{Susp-Ctx} cannot be applied.
\end{example}
...
let ex_7 r = let x = r.x in x + r.y °\ocamlflags 10°
\end{program}
The constraint generated when typing \code{ex_7} contains the following, where $\tv$ stands for the type of \code{r}:
\begin{mathpar}
  \cexists \tv
    \clet x \tvb
      {(\cmatch \tva \dots)}
...
\tint \cand \cunif \tv {\mathsf{one}}$. Although the context includes a
\code{let}-binding --- which in practice involves \code{let}-generalization --- we
can still deduce $\Cshape \C \tv {\mathsf{one}}$, since the erased context $\cerase \C$ contains the unification $\cunif \tv
{\mathsf{one}}$.
...
{\capp {getx} {(\tint \ \mathsf{two} \to \tv)}}$. Since $getx$ is bound to the constraint abstraction $\cabs \tvd \exists \tvc.\uad \cunif \delta {(\gt \to \gamma)}$,
  the instantiation $\capp {getx} (\tint \ \mathsf{two} \to \tv)$ can only be satisfied when $\gt = \tint\ \mathsf{two}$. This proves unicity, hence \code{ex_7} is accepted by our semantics.
\end{example}
...
\begin{lemma}
  ...
  \end{mathpar}
  \begin{proof}
    Similar proof to \cref{lem:unicity-match-type}.
  \end{proof}
\end{lemma}
...
\begin{proof}
  ...
  \begin{proofcases}
	...
	\begin{llproof}
	  ...
	  \equivPf{\upa}{\upb}{\cref{lem:unification-preservatoin}}
	  \simplePf{\upa, \upb}{\cref{lem:unif-problem-simple}}
\Hand 	  \equivctxPf{\upa}{\upb}{\cref{lem:equiv-ctxt-simple}}
	\end{llproof}
...
\Hand	    \vdashPf{\semenv}{\cletr \x \tv \tvs \ueqs \c}{By \Rule{LetR}}
  ...
  \TODO{Proof case for \Rule{S-Exists-Lower}}
  \proofcaserewrite
    {S-BackProp}
    {\C\where
       {\cletr \x \tv \tvs {\Ca\where{\cmatch \tvp \cbrs}}
  ...
  \begin{llproof}
    Equivalences are standard, see \citet{Pottier-Remy/emlti}.
  \end{llproof}
  \end{proofcases}
\end{proof}
...
\begin{lemma}[Unification progress]
  ...
  \begin{enumerate}[(\roman*)]
    ...
    \end{enumerate}
  \begin{proof}
    This is a standard result. See \citet{Pottier-Rem/emlti}.
  \end{proof}
\end{lemma}
...
\begin{proof}
  ...
  \TODO{Proof case for match constraints}
  \begin{proofcases}
...
\begin{lemma}
  ...
  $\C\where{\cmatch \t \cbrs}$ is unsatisfiable if $\nCshape \C \t \sh$ for any shape $\sh$.
  \TODO{This lemma is wrong. We need a stronger condition on progress}
\end{lemma}
