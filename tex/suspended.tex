...
given on \cref {fig/typing/core}, are just \ML typing rules.
We then present then first, followed by rules for explicit version
of fragile construts, which are part of folklore knowledge.
Finally, we present the typing rule for implicits fragile constructs, which
are the crux of our work.

\subsubsection {\ML typing rules for \ML and \OML robust extensions}
...
\paragraph {\OML extensions with explicit typing}
...
technical.  They they use auxiliary typing judgments $\el : \t \to \tp$ and
$\G \th \el = \e : \t$ for the typing of labels and
record assignments, respectively.
\XDR
  {I find rules for records a bit messy. Rules for explicit records should
  be easier to write, and we should not need \Rule {Rcd-Assn}, just
  $\parens{\G \th \ei : \ti}\iton$ and changing Rule \Rule {Rcd} to have
  $\iparens{\eli : \ti \to \t}$ and $\bar \el \uni \t$ for premises.}
%
We assume that record types have been previously defined, building a global
environment $\labenv$ that maps pairs of record labels and record types
$\elab/\T$ to the type of the corresponding projection functions.
%
Notice that $\el$ stands for a label $\elab$ alone or a label with an
explicit type $\T$ written $\elab/\T$.
%
While rules \Rule {Rcd} and \Rule {Rcd-Proj} applies to both explicit and
implicit use of labels, \Rule {Lab-X} only allows to assign types to
explicit labels so far.
\XDR
  {I actually don't understand the use of the uniqueness predicate in
  explicit rules: labels $\elab/t$ should be unique in type $\T$ by
  construction.}
\XDR{To fix and finish}

\subsubsection {Typing of \OML fragile implicit constructs}

\begin{mathparfig}[t]{fig/typing/I}
        {Typing rules of \OML---fragile implicitly typed extensions.}
  \inferrule[Proj-I]
    {\eshape \E \e {\any \tvcs \Pi\iton \tvcs} \\
...
\parcomment {Structual tuples}

\subparagraph{Overloaded tuples}
