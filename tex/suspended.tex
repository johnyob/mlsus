...
\footnote
  ...
  constraint of the body before they are fully sovled, but this copies the
  whole constraint, including unsolve parts and duplicates the work of the
  constraint resolution. That is, they still lack a mechanism to
  incrementally and efficiently instantiate partial type schemes.
%%
...
\XDR{This footnote could be inlined}
\relax
...
of the form $\all \tv \T \tys \to \t$.\XDR{I inverted the convention, since
a projection type makes sense, but an injection type does not for record
fields.} A label $\elab$ may belong to several record types, but is unique
...
either $\elab$ or $\elab/\T$ or $\lab \el$ when $\el$ is $\parens {\elp :
\exi \tvs \t}$.  We write $\parens {\el \labin \Labenv}$ for a generalization of
...
\end{mathpar}
Similarly, we write $\parens {\els \labof \Labenv}$ the generalization of
...
to check that labels $\el$ contains all labels of the record type $\t$ and
are altogether uniquely determined by type $\t$, as explained next.
...
reasoning, based on the available definitions available in the global
environment $\labenv$:
\relax
...
We first uniqueness with respect to first record type \emph{names}, then
to record types of such names.
\begin{mathpar}
\def \Wide#1{&#1&}
\begin{tabular}{R;;C;;L}
