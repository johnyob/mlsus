...
\Xalistair{This subsection isn't really saying anything new. The reader should know this by
now. I suggest we cut this section (maybe reminding the reader of the
constraint generation function in the next subsection). }

Our constraint generation is defined as follows. Given a term $\e$ and
its expected type $\tv$, we
define a constraint $\cinfer \e \tv$ that is satisfiable if and only if
the term is well-typed for some instantiation of $\tv$.
The constraint generation for implicit constructs use
...
\Xalistair{I'm pretty sure we have factorization with this statement
 (since it suggests the overloaded constructs in $\e_0$ are resolved by $\e_0$
 alone). I think if we remove $\G \th \e_0 : \ts$, then we have the argument
 we want. }
...
system, operating modulo $\alpha$-equivalence, and associativity and
...
\Xgabriel{If you decide to move this section back in, you should explain why we care about this feature.}
