...
let-bindings $\elet \x \ea \eb$: first typecheck the defintion $\ea$, then
...
This is becuase the \code{let}-binding in \code{ex_2} allows \code{r} to be
treated polymorphically, and thus its type is considred \emph{known}--enabling
...
semi-explicit first-clas polymorphism, and later used by~\citet
{LeBotlan-Remy/recasting-mlf} for empowering \MLF. It has since been adopted in
...
reasoning and proofs. On the upside, the semantics directly suggest declarative
typing rules for the surface language.
...
the canonical shape of $\t$, and the second conjunct is the selected branch
constraint $\ci$ under the appropriate substitution. Since the syntax of suspended match
constraints requires that branch patterns are non-overlapping, the
matching branch $\cbranch \cpati \ci$ is uniquely determined; but it
may not exist as branches need not be exhaustive, in which case the discharged
constraint is $\cfalse$.
\end{definition}
