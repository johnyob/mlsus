...
annotations. In order to minimize the need for type anotations, their
specifications have been twisted with some direct or indirect algorithmic
flavor in order to preserve \emph{principality}.
%
...
examples whose type could easily be guessed.\Xgabriel{I would like to explain
more here, it is not obvious why completeness-breaking examples also endanger
principality.} \Xalistair{Perhaps we pick an example extension}
Unsurprisingly, they have historically posed challenges to the preservation of
principal types.
...
\texttt{match} scrutinee and return type can be annotated to a rigid type which will be
...
\entry[Patterns]{\cpat}{}{} \\
%% \entry[Constraint abstractions]{}{\clam \tv \C}
%% \\
\entry[Shapes] \sh {} {}
\end{bnffig}
...
that the conclusion of \Rule{Susp-Ctx} $\C[\cmatch \tv \cbrs]$ is structurally
...
context of the suspended constraint requires $\tv$ to be assigned to some
...
apply the assignment of $\tva$ to $f$. More formally,
\begin{mathpar}
  \inferrule*
    {\semenv(\tva) = \overline{\t} \Fapp \\ \semenv_{\setminus
    \Delta}[\overline{\tvb := \t}] \vdash f(\overline{\tvb} \Fapp)}
    {\semenv \vdash \cmatch \tva \Delta f}
\end{mathpar}
However, our informal solver would be incomplete with respect to this
semantics.  We show this by considering the constraint $\cexists \tv
\cmatch \tv \tv (\lambda \_.~ \tv = 1)$.  This constraint does not
unify $\tv$ outside the suspended constraint, but once it is scheduled,
the variable $\tv$ must be unified with the unit type $1$. So our
informal solver suggests that this constraint is unsatisfiable since
$\tv$ is never unified outside the match constraint. Yet we can use the
      ...
      \inferrule*
        {
          (\tv \mapsto 1)(\tv) = 1 \\
          \inferrule*
            {(\tv \mapsto 1)(\tv) = 1} {\tv \mapsto 1 \vdash \tv
            = 1}
        }
        {\tv \mapsto 1 \vdash \cmatch \tv \tv (\lambda \_. ~ \tv
        = 1)}
    }
    {\cdot \vdash \cexists \tv \cmatch \tv \tv (\lambda \_.~ \tv = 1)}
\end{mathpar}
The dependency of suspended constraints can also be complex, as in the
following example: $\cexists {\tva, \tvb} \cmatch \tva \tvb {(\lambda
\_. ~ \tvb = 1)} \cand \cmatch \tvb \tva {(\lambda \_. ~ \tva = 1)}$.
Worse still, is that the binding location of $\tva$ may not even contain
the sufficient information to show that $\tva$ is realised.  For instance,
\begin{align*}
  \cexists \tvb \tvb = 1 \cand \cexists \tva \tva = \tvb \cand \cmatch \tva \Delta f
\end{align*}
...
variable $\tv$ by some constraint context $\cctx[-]$.

\begin{definition}
  A type variable $\tv$ is principally realised with the constructor $\F$
  by the context $\cctx[-]$, written $\cctx[\tv !] \rhd \F$,
...
\inferrule*
   {\cctx[\tva !] \rhd \F \\
    \semenv \vdash \cctx[\semenvp_{\setminus \Delta}[\overline{\tvb :=
      \t}] \vdash f(\overline{\tvb} \Fapp)] \\
    \semenvp(\tva) = \overline{\t} \Fapp}
   {\semenv \vdash \cctx[\semenvp \vdash \cmatch \tva \Delta f]}
\end{mathpar}
...
\item and finally the global rule
  \begin{mathpar}
    \infer
    {\c[\tva !]
      \and \semenvp(\tva) = \overline{\t} \Fapp
      \and \semmetaenv, (\semenvp, \overline{\tvb := \t} \vdash
      \square)\vdash \c[f(\overline{\tvb} \Fapp)] }
    {\semmetaenv, (\semenvp \vdash \square); \semenv \vdash \c[\cmatch \tva \Delta f]}
  \end{mathpar}
\end{itemize}
...
(\cref{fig:omni-solver}). These rulesdefine a non-deterministic rewriting
...
block box by the solver, allowing the solver to be parameterized over the
...
In general, existential quantifiers $\cexists \tv \c$ are lifted to the nearest enclosing
$\Let$, if any, or otherwise to the top of the constraint.
...
and $\tvs$ are auxiliary existential variables. The order of $\tvs$ is
immaterial, and regions are considered equal up to permutation of these
variables.
...
\Rule{S-Let} rewrites the usual let constraints $\clet \x \tv \ca \cb$ into
region form. \Rule{S-Exists-Conj} lifts existentials across conjunctions,
while \Rule{S-Exists-Let1} and \Rule{S-Exists-Let2} lift existentials into or
across let-binders. \Rule{S-Let-Conj1} and \Rule{S-Let-Conj2} hoist
constraints out of let-binders when they do not depend on locally bound
variables. Collectively, these lifting rules normalize the structure of each
region into a block of existentially bound variables, comprising set of solved
multi-equations (from unification) and residual unsolved constraints (\ie
applications, let-bindings and suspended constraints).
...
constraints, introduced in \cref{TODO}, are handled by rewriting them once
their left-hand side becomes concrete. For instance, an instantiation
constraint $s \leq \t$ is handled once $s$ is subsitituted by a match
constraint, yielding $\ts \leq \t$, which is then solved using the syntactic
sugar introduced in \cref{sec:constraints}. Similarly, label instantiation
...
\begin{mathparfig}
  ...
  \rewrite[S-Susp-Ctx]
    {\C[\cmatch \tv \cbrs] \cand \cunif \tv {\cunif \t \ueq} \\
     \shape \t = \any \tvcs \tp \\
     \tv \disjoint \C}
    {\C[\cexists \tvcs  \cunif \tv \tp \cand \cmatch \tp \cbrs]
     \cand \cunif \tv {\cunif \t \ueq}}
...
\begin{mathpar}
  \rewrite[S-Let-App-$\beta$]
    {\cletr \x \tv \tvs \ca {\C[\capp \x \t]} \\ \tv, \tvs \disjoint \t \\ x \disjoint \C}
    {\clet \x \tv \ca {\C[\cexists {\tv, \tvs} \cunif \tv \t \cand \ca]}}
\end{mathpar}
While obviously correct, this na\"ive strategy duplicates constraint solving
work across applications of the same abstraction.

% Background on efficient solving of applications (aka generalization)
...
\begin{mathpar}
  \cexists \tv
    \clet y \tvb {\cmatchdots \tva }
      {\cinst y \tint \cand \cunif \tv {\mathsf{three}}}
\end{mathpar}
Here, $\tv$ is \code{r}'s type. The constraint remains
suspended until \code{r.y} forces \code{r}'s type to be \code{three}.
...
\code{y}. We na\"ive pick $\tfor \tvb \tvb$. This appears unsound, since
...
%
This motivates our second novel mechanism for omnidirectional inference:
\emph{partial type schemes}---type schemes that delay commitment to certain
quantifications (\eg $\tvb$). Such \emph{partially generalized} variables are
treated as generalized, but can still be refined in future as suspended
constraints are discharged.

% Intro partial applications
...
\begin{mathparfig}
  ...
  \rewrite[S-Let-App]
    {\cletr \x \tv \tvs \ca \C[\capp \x \t] \\
     \tvc \disjoint \t \\
     \x \disjoint \C}
    {\cletr \x \tv \tvs \ca \C[\cexists \tvc \cunif \tvc \t
                               \cand \cpapp \x \eset \eset \tvc ]}

  \rewrite[S-Papp-Exists]
    {\cletr \x \tv {\tvs, \tvb} \c \C[\cpapp \x \ren \ueqs \tvc] \\
     \tvb \notin \dom \ren \\
     \tvbp \disjoint \ren, \ueqs, \tvc \\
     \x \disjoint \C}
    {\cletr \x \tv {\tvs, \tvb} \c
      \C[\cexists \tvbp \cpapp \x {\ren \where {\tvb \is \tvbp}} \ueqs \tvc]}

  \rewrite[S-Papp-Unif]
    {\cletr \x \tv \tvs {\c \cand \ueq} {\C[\cpapp \x \ren \ueqs \tvc]} \\
     \ren : \tvs \\
     \ueqs \nvDash \ueq \\
     \x \disjoint \C}
    {\cletr \x \tv \tvs {\c \cand \ueq}
      \C[\cpapp \x \ren {\ueqs, \ueq} \tvc
	 \cand \ueq[\ren\where{\tv \is \tvc}]]}

  \rewrite[S-Papp-Solve]
    {\cletr \x \tv \tvs {\ueqs'} {\C[\cpapp \x \ren \ueqs \tvc]} \\
     \ren : \tvs \\
     \ueqs \vDash \ueqs' \\
     \x \disjoint \C}
    {\cletr \x \tv \tvs {\ueqs'} {\C[\ctrue]}}
  ...
  \rewrite[S-Exists-Lower]
    {\cletr \x \tv {\tvas, \tvbs} \c
     {\C[\overline{\cpapp \x \ren \ueqs \tvc}]} \\
     \cdetermines {\cexists {\tv, \tvas} \ca} \tvbs \\
     \bar \ren : \tvas, \tvbs \\
     \x, \tvbs \disjoint \C}
    {\cexists \tvbs
      \cletr \x \tv \tvas \c
	{\C[{\overline{\cpapp \x {\ren \setminus \tvbs} \ueqs
...
\begin{mathpar}
  \infer[Papp]
    {\semenv, \tv \is \semenv(\t) \th \ueqs [\ren]
	      \implies \semenv \th \capp \x \t \\
     \semenv(\x) = \cabsr \tv \tvs \c \\
     \dom \ren \subseteq \tvs}
...
variables modulo $\alpha$-equivalence. Interestingly, $\tvs$ are bound not
...
\begin{enumerate}
  \item \Rule{S-Papp-Exists} allocates fresh existential variables at the call
    ...
    We write $\ren : \tvs$ for $\dom \ren = \tvs$.

  \item \Rule{S-Papp-Unif} copies multi-equations from the abstraction
    to the call site, provided we have not copied them previously.
\end{enumerate}
% Cleaning up partial applications and let constraints
\Rule{S-Papp-Solve} removes partial applications once all necessary
constraints have been copied -- that is, when the copied equations entail the
abstraction's body. \Rule{S-Let-Solve} remove a $\Let$ constraint when the
bound term variable is unused and the abstraction is satisfiable.

% Lowering
\Rule{S-Exists-Lower} implements the non-trivial case of lowering existentials
across $\Let$-binders. It identifies a subset of variables in the region
of a $\Let$ constraint that are unified with variables from outside the
region. Such variables are considered monomorphic and thus cannot be
generalized; they can instead be safely lowered to the outer scope.
...
\begin{definition}
  $\cdetermines \c \tvbs$ if and only if for every ground assignment
  $\semenv, \semenvp$ that satisfy $\c$ and coincide outside of $\tvb$ must
...
% How the determines relation corresponds to ML
...
% Why we lower?
Lowering such varibles improves solver efficiency. It avoids unnecessary
...
% Updating partial applications
However, sometimes \Rule{S-Papp-Exists} must copy a variable before it can be
lowered, for instance, in the constraint $\cexists a b $\TODO{Think of
constraint that forces our solver}. In this case, when we eventually lower the
...
%\begin{mathpar}
%  \rewrite[S-BackProp]
%    {\C[\clet \x \tvs {\Ca[\cmatch \tv \cbrs]} {\Cb[x[\theta \th \up]]}] \\
%     \theta : \tvs \\
%     \C[\Cb] \th \theta(\tv) = \t \\
%     \shape \t = \any \tvcs \tp}
%    {\C[\clet \x \tvs {\Ca[\cexists \tvcs \cunif \tv \tp
%                           \cand \cmatch \tp \cbrs]}
%		      {\Cb[x[\theta \th U]]}]}
%\end{mathpar}
%
