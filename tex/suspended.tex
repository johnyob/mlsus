...
which define overlapping field names:
\begin{program}[input]
...
\Xdidier[[On the upside, the semantics directly
  suggest declarative typing rules for the surface language.]  {Repeating
  line 170}
...
``guess''\Xdidier{I would prefer to replace ``guess'' by ``assignment'' here
and next occurrence} for each free variable of $\c$ (type and term
variable), and $\semenv \th \c$ states that these guesses indeed satisfy
$\c$. Let us write $\Ground$ for the set of \emph{ground} types, types
without free variables.\footnote{Ground types are thus finite trees,
...
constraint-formers\Xdidier {Why a dash in constrain-formers?} except
suspended constraints; its definition on this fragment is standard and
somewhat tautological. The constraint $\ctrue$ is satisfied by any
environment, and $\cfalse$ by none. An environment $\semenv$ satisfies $\ca
\cand \cb$ if it satisfies both $\ca$ and $\cb$. Satisfying $\cexists \tv
\c$ requires finding a witness $\gt$ for $\tv$.  The universal constraint
$\cfor \tv \c$ is satisfiable if $\c$ is satisfiable for any binding of
$\tv$. The unification constraint $\cunif \ta \tb$ is satisfied when
...
unification.
\Xdidier[\cref{TODO}]{What citation did you have in mind? Is it
needed?}\Xdidier{We never clearly define unification on shapes.}
...
\begin{lemma}[Unification termination]
  ...
  \begin{proof}
	...
	So we are done.
        ...
        \Xgabriel{I don't follow, for example if $\tau_i$ is a nullary shape
        $\shapp \eset$ I would expect $1 + \tw {\shapp \eset} = \iw {\shapp
        \eset}$.}
