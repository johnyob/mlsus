...
\begin{definition}
  ...
  \cerase {\C\where{\cunif \tv \gt}}$ implies that the principal shape of $\gt$ is equal
  to $\sh$.
%
  In other words:
  \begin{mathpar}
    \Cshape \C \tv \sh \Wide\eqdef \forall \semenv, \gt. \uad
      \semenv \vdash \cerase {\C\where{\cunif \tv \gt}} \implies \shape \gt = \sh
  \end{mathpar}
  ...
  The use of the erasure function $\cerase \c$ (defined in
  \cref{fig:constraint-erasure}), which erases match constraints with a
  variable in the matchee $\cmatch \tv \cbrs$, ensures that unicity is proven
  with respect to what is \emph{known} at present. Match constraints can
  represent constraints which are \emph{possibly} true in the future and should not be
  used to prove what is known about the present.
\end{definition}
...
\newcommand{\Erule}[2]{\cerase {#1} &\eqdef& {#2}}
  ...
  \Erule{\capp \x \t}{\capp \x \t} \\
  \Erule{\cmatch \t {\cbranch {\bar \cpat} {\bar \c}}}{\begin{cases}
    \ctrue &\text{if } \t \in \TyVars \\
    \cmatch \t {\cbranch {\bar \cpat} {\cerase {\bar \c}}} &\text{otherwise}
  \end{cases}} \\
\end{tabular}
\end{mathparfig}
...
%
However, this negative occurrence means that, \emph{a priori}, the rules
...
\Cshape \C \tv \sh \Wide\eqdef \
  \forall \semenv, \gt. \uad
      \semenv \th \cerase {\C\where{\cunif \tv \gt}} \implies \shape \gt = \sh
\end{mathparfig}
