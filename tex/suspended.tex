...
order.  Let $\typp$ be $\tp \where {\tvbs \is \tvas}$.
\relax
We now have $\tpoly {\all \tvs \t}$ equal to
$\tpoly {\all \tvs \typp \where {\tvcps \is \typs}}$~\llabel{PP},
which implies that $\t$ is equal to $\typp \where {\tvcps \is
\typs}$~\llabel T.
\relax

\medskip

Since $\typp$ does not contain any variable in $\tvs$, every path $\pi_i$
ends in $\typp$.
\relax
Hence, we have $\typp/\pi_i$, say $\tyip$ is equal to
$\typp \where {\tvcps \is \typs}/\pi_i$, \ie. $\tyz/\pi_i$.
We can write $\typp$ as
\relax $\typp \where {\pi_i \is \tyi''}\iton$ where $\tyi''$ is $\typp/\pi_i$.
This is also equal to
\relax $(\typp \where {\pi_i \is \tvci}\iton) \where {\tvci \is \tyi''}\iton$,
\ie. $\tyz\where {\tvci \is \tyi''}\iton$.
In summary, we have $\typp = \tyz \where {\tvci \is \tyi''}\iton$.
\relax
This implies that
 $\tpoly {\all \tvs \typp}$, \ie
 $\tpoly {\all \tvbs \typ}$ is equal to
 $\tpoly {\all \tvs {\tyz \where {\tvci \is \tyi''}\iton}}$, \ie
 $\tpoly {\all \tvs \tyz} \where {\tvci \is \tyi''}\iton$~\llabel E.
%
...
substituting solved existential variables. We write $\sub(\t)$ for $\sub$
