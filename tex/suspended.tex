...
in~\Cref{fig:constraint-syntax}. We write $\Types$ the set of types and
$\Typesz$ the subset of nonvariable types.
%
...
wrote $\clet \x \tv \Cone \Ctwo$ instead of $\Clet \x \tv \Cone \Ctwo$.
[It is macroyfied, so easily reversible.]}
...
\Xgabriel{Each new language feature would need to introduce its own
domain-specific constraint formers; for example to type nominal records you
need to fetch field declarations from the declaration environment. I would
rather leave this to the meta-level to keep the core language
simple.}\Xdidier{That is not good reason: we may have a concrete language
with dots and exhibit the properties that should holds for the language
and do hold for the current features.}
\Xalistair{I agree with Didier here, I've switch over to an explicit
pattern representation in my first year report and its not any worse
and we can easily paramterize the semantics by a 'matches' predicate.
Moreover, this ensures we have well-foundness (something meta-functions
don't necessarily break, but complicate) and termination of the solver. }
...
Informally, the shape of a variable is undefined and the shape
of a type $\tys \F$ is its symbol $\F$, but we prefer to write is
as a linear shallow type $\exi \tvs \tvs \F$.

\TODO{Fix this:}
Given a nonvariable type $\t$, we
write $\t =  \deshaped [\tvs] {\tvs \F} \tys$ when $\\exi \tvs \tvs \F$ is
the shape of $\t$ and $\t$ is equal to $\tys \F$.
...
\end{mathpar}
\Xdidier {$\mlet \ts {\Omega(\elab / \F)} {\ts \leq \tva \to \tvb}$ looks
like a concrete constraint, but it is not a syntactically well-formed
one. In fact, it is part of the meta-language, hence I changed the style.}
...
We introduce \emph{shapes}, a generalization of the notion of type
constructors to cope with polytypes.  A shape $\sh$ is
an existentially quantified type $\any \tvs \t$ where $\tvs$ is exactly the
sequence of free variables of $\t$ in order of appearance in $\t$.
...
carry the idea than \texttt{\string\any} for the macro name.} which work
Hences, shaped are closed. They are treated up to $\alpha$-conversion.
...
\end{theorem}
We write $\shape \t = \sh$ for the non-trivial principal shape of $\t$ (if
defined). The $\shape {\tys \F}$ is $\exi \tvs \tvs \F$.
The interest of the definition is to extend to polytypes:
...
\paragraph {Suspended constraints, an attempt}
...
\begin{mathpar}
\infer[Susp-Nat]
  {\semenv(\t) = \deshaped [\tvs] \typ \tys \\
   \semenv_{\mid \Delta}, \tvs \is \tys \th f(\tp)}
...
\begin{mathpar}
  \infer*[Right=Conj]
   {\infer*[Left=Unif]
    ...
    \infer*[Right=Susp-Nat]{\deshaped \tint = \angles {\tint, []} \\
...
The example would perhaps work better with structural tuples?}
...
\Xgabriel{Shouldn't we ask for a non-trivial shape in this
rule?}\Xdidier{Yes, indeed}:
\begin{mathpar}
  \infer[Susp-Use]
    {\t = \deshaped [\tvs] \typ \tys \\
    \typ \in \Typesz \\
     \semenv_{\mid \Delta},\tvs \is {\semenv(\tys)}  \th f(\tp)
    }
    {\semenv \vdash \cmatch \t \Delta f}
\end{mathpar}
...
statically. Hence, it does not allow to guess.
...
\begin{mathpar}
  \infer[Susp-Ctx]
    {\Cshape \C \tv {\exi \tvs \t} \\
      \semenv \th
         \C \where {\cexists \tvs \cunif \tva \t \cand \cmatch \t \Delta f}
    }
    {\semenv \vdash \C \where {\cmatch \tv \Delta f}}
\end{mathpar}
...
%
The side-condition $\Cshape \C \tv \sh$ ensures this
precisely: it expresses that the context $\C$ constrains all the solutions
of $\tv$ to have the \emph{same} shape $\sh$, modulo $\alpha$-equivalence of
shapes. The formal definition of this condition, which is somewhat
technical, will be introduced later.
...
%
  ...
  \Xdidier  [That’s what justifies applying \protect\Rule{Susp-Ctx}.]
...
there is no recursion in the language of constraints}.
...
\begin{mathpar}
  \cexists \tv
    \clet x \tvb {\cmatch \tva {\tvb} {(\lambda (\tvcs \Fapp).~ \ldots )}}
...
\Alt{
  ...
  \infer[Susp-Ctx]
    {\Cshape \C \tv \sh \\\\
      \semenv \th \C[\cexists {\fvs \sh} \cunif \tv \sh \cand
      \cmatch \sh \Delta f]
    }
...
\Cshape \C \tv \sh \Wide\eqdef \
  \forall \semenv, {\any \tvcs \t} \in \Typesz,
      \semenv \th \C[\cexists \tvcs \cunif \tv \t] \implies
            \sh \le \any \tvcs \t
\end{mathparfig}
...
\begin{theorem}
All equivalence rules preserves the solutions.
\end{theorem}

Define a strategy by orienting the rules.
\begin{theorem}
The stategy always terminates.
\end{theorem}

\begin{theorem}
All constraints in normal forms, but $\cfalse$ admit principal solutions.
\end{theorem}
...
For sake of simplicity, we have taken finite trees for ground types, hence
the semantics of constraints. Taking regular trees instead would have worked
as well, since we never assume finiteness of ground types, as in \cite
{Pottier-Remy/emlti}. This would model equirecursive types.
