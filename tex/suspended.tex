...
Given a shape $\Shp \tvcs \t$, we write $\deshaped [\tvcs] \t \tys$ for $\t
...
\Xdidier
[A shape $\any \tvs \t$ is canonical if the sequence of free variables
$\tvs$ is sorted by some arbitrary ordering on bound variables]{I disagree:
shapes to be $\alpha$-convertible, which does not preserve this
ordering. Hence I reversed the definition.}  A principal shape $\any \tvs
\t$ is canonical if the sequence of its free variables $\tvs$ appear
in the order in which the variables occur in $\t$.\footnote{This order is
well-defned for principal shapes, but not always well-defined for all
shapes\Xdidier{What example do you have in mind?}}  We write $\sh$ for
canonical principal shapes.
...
shape, if it exists, and $\pshapp \tys$ for the recomposition of a type where
$\sh$ is a canonical principal shape. \Xdidier {Too many notations.
I would not use $\pshapp$, just require that $\sh$ only range over principal
shapes. We could keep $\sh$ for principal and use $\zeta$ (or $s$ any shape,
or inline them.}
...
\Rule{Susp-Ctx} rule is therefore satisfied, and the suspended constraint
can be resolved.
...
As we have previously hinted, the semantics for a suspended constraint must
