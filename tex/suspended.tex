...
%% , \ie from which all other valid types for $\e$ are instances of $\ts$.
...
\textbf{i}nstances. This strategy, first introduced
by~\citet*{Garrigue-Remy/poly-ml} for polytypes, has later been reused for other
\OCaml fragile constructs.
...
\XDR
  ...
  to counter-intuitive behavior.]  {I changed it to} Although it ensures
  principality, it may sometimes be unintuitive for the user.

\XDR
  {Still, I dislike the huge rewriting of this part, which starts by to saying
  that \Geninst-directional type inference is counter-intuitive! I just
  \emph{fully} disagree. We discussed it during one and I
  agreed to say it mindly, but the new rewrite make is look like a bad
  feature. I think this is denying the true, and misleading.
  The good of the paper is not to kill \geninst-directional type inference,
  to sell omni-directional type inference... }

For instance, consider the following two record types with overlapping
field names:
\begin{program}[input]
...
resolution strategy instead of failing when the type is ambiguous,
  ...
  the warning on default resolution.  Warnings are shown in
  \smashcolorbox{orange!20}{\strut orange} and should be treated as failures in
  \smashcolorbox{red!20}{red}.}
\relax
...
\ocaml{ex_3}, where \texttt{@@} and \ocaml{|>} are the application and
reverse application functions:
\begin{program}[input,check]
let ex_3_2 = (fun r -> r.x) @@ one                         °\ocamlflags 10°
let ex_3_3 = one |> (fun r -> r.x)                         °\ocamlflags 20°
\end{program}
...
algorithm, depending on whether the function or argument is typed first.
To limit such implementation-dependent behavior, \OCaml infers
all subexpressions until they are \code{let}-bound, and warns
when using disambiguation before the expression is let-bond.
Consequently, \OCaml does not makes little difference between
\ocaml[indices]{ex_3}, \ocaml[indices]{ex_3_2}, or \ocaml[indices]{ex_3_3},
just warning instead of failing on \ocaml[indices]{ex_3_3}, which could all
be considered as ill-typed.

This criterion also warns on example \ocaml[indices]{ex_4_1} below, where
\code{r} has a monomorphic type and indeed.
\begin{program}[input,check]
let ex_4_1 p r = if p then (r : 'a one).x else r.x         °\ocamlflags 20°
let ex_4_2 p r = if p then r.x else (r : 'a one).x         °\ocamlflags 10°
\end{program}
Indeed, its variant \ocaml[indices]{ex_4_2} fails to typecheck.
...
\ocaml{(fun r -> r.x)} in bidirectional fashion, which would otherwise be
...
default. The user must explicitly pass the \texttt{\small -principal} flag
to require the more expensive computation when desired.
...
to explain the general framework of suspended constraints. For now, it sufficies to
...
\section*{\Large Organization of our appendices}
\label{app:outline}
...
nominal records, structual tuples, and polytypes.
We present is typing rules in \cref {app/onml/typing},
and illustrate them on examples in \cref {app/oml/examples}.
Finally, we show constraint generations that implements the typing rules
in \cref {sec/oml/constraints}. The metatheretical resuls for \OML
are presented in \cref {sec/oml/metatheory} and provewd in
\cref {app/oml/proofs}.
...
\OML extends \ML with new features. Hence, core \OML typing rules,
given on \cref {fig/typing/core}, are just \ML typing rules.  We then
present then first, followed by rules for explicit version of fragile
construts, which are part of folklore knowledge.  Typing rule for implicits
fragile constructs, which are the crux of our work, are deferred to the next
section.
...
\begin{mathparfig}[t]{fig/typing/X}
        {Typing rules of \OML---robust, explicity typed extensions.}
  \inferrule[Tuple]
...
Each fragile extensions $\e^i$ comes with a robust explicit associated
construct.  We first present the typing of explicit constructs $\e^x$ on
\cref {fig/typing/X}, namely structural tuples, polytypes, and nominal
records.  Their typing is unsurprising and part of folklore knowledge.
Still, we leave defer the presentation of nomimal records to the next
section (\cref {app/typing/X/records}), as they are longer to explain.
...
when the annotation is a closed type schems $\ts$, Rule \Rule {Poly-X} just
...
\XDR
  {I find rules for records a bit messy. Rules for explicit records should
  be obvious to write/read.  We should not need the intermediate rule \Rule
  {Rcd-Assn}, just $\parens{\G \th \ei : \ti}\iton$ and changing Rule \Rule
  {Rcd} to have $\iparens{\eli : \ti \to \t}$ and $\bar \el \uni \t$ for
  premises.  They is also an abuse of notations with $\el/\T \in \Omega$
  where $\in$ is both interpreted as math membership and an inductively
  define predicate with math membership as an implicit axiom}

\XDR
  {I propose to get read of \Rule {Rcd=Assn}}
...
\to \tp$ for the typing of labels.
Label expressions $\el$ extend labels with explicit labels $\elab/\T$
and annotated label expressions $\el : \exi \tvs \t$ (where the annotation
the shape of the record type they belong to).
\begin{version}{}
\begin{mathpar}
\begin{bnfgrammar}
...
environment $\labenv$ that maps labels to their projection type, \ie of the
form $\all \tv \T \tys \to \t$.\XDR{I propose to invert the convention.} For
a given record type $\T$, we assume each label associated with it is
unique, while labels may belong to several record types.

We write $\Labenv$ for the restriction $\set {\elab : \all \tvs \T \tys
\to \t \in \labenv}$ of $\labenv$ to labels of type $\T$. Hence, $\dom
(\Labenv)$ is the set of labels belonging type $\T$.
%% We write $\Labenv{} (\elab/\T)$ for $\Labenv \elab$---
Hence, $\labtyp \elab$ is the (unique) type scheme $\ts$ associated to
$\elab$ in type $\T$.
%
We write $\lab \el$ the label underlying $\el$, \ie $\elab$ when $\el$ is
either $\elab$ or $\elab/\T$ and $\lab \el$ when $\el$ is $\elp : \exi \tvs
\t$.  We define $\el \labin \Labenv$ as a generalisation of
$\elab \in \dom(\Labenv)$ as follows:
\begin{mathpar}
...
\end{mathpar}
We write $\els \labof \Labenv$ the generalisation of
$\bar \elab = \dom (\Labenv)$ defined as
\begin{mathpar}
\els \labof \Labenv \Wide \eqdef
        \lab \els = \dom (\Labenv) \wide\wedge
        \forall \el \in \els,\; \el \labin \Labenv
\end{mathpar}
Rule \Rule {Lab} extracts the type sheme of a label $\elab$ from
$\labenv$. Rule \Rule {Lab-X} does the same but for an explicit label
$\elab/\T$. Rule \Rule {Lab-Inst} allow instantiation of the type of label
expressions. Rule \Rule {Lab-Annot} is an alternative to the use of explicit
labels $\elab/\T$ that allows to constaint the type of a label expression.
...
projection. In addition, the premise $\el \uni \t$ checks that the label
...
to check that both labels $\el$ contains all labels of type $\T$ and can
altogether only belong to type $\T$.

In the explicit case, this could just require that $\els$ contains an
explicit label expression $\elab/\T$. However, we wish uniqueness to also be
determined by annotations and support \emph{closed-world} reasoning, based
on the available definitions available in the global environment $\labenv$:
\begin{enumerate*}
\item
...
\begin{mathpar}%
  \el \uni \T \Wide\eqdef
  \el \labin \Labenv \wide\wedge
  {\forall \Tp, \uad \el \labin \Labenv[\Tp] \implies \T' = \T}

\el \uni {\T \tys} \wide\iff \el \uni \T

\els \uni \T \Wide\eqdef
  \els \labof \Labenv \wide\wedge
  \forall \Tp, \; \els \labof \Labenv[\Tp] \implies \Tp = \T

  \els \uni {\T \tys} \wide\iff
  \els \uni \T
\end{mathpar}
These predicates only depend on the global environment $\labenv$.
They just ensures that this is actually no real overloading involved.
...
\cref{fig/typing/I}. While the typing
rules for fully annotated terms ($\e^x$) are unsurprising, and part of
folklore knowledge, the typing rules for terms with omitted type annotations
are quite new and non-compositional.
%
They all share a common framework that ensures that some types are uniquely
determined from the context. Still each construct requires a specific
...
\end{bnfgrammar}
\end{mathpar}
As with our contextual rules for expressions,
we define two rules for labels.
...
%% environment $\labenv$.
we introduce a rule \Rule{Lab-I} that handles unannotated labels by elaborating
...
\end{mathpar}
Since we extended  $\el$ to also contain $\elmagic \elab$, we
define $\lab {\elmagic \elab}$ as $\elab$ and
$\elmagic \elab \labin \Labenv$ as $\elab \labin \Labenv$.
...
For that purpose we generalize the defintion of $\el \uni \T$ and $\els \uni
...
\XDR{I don't undertand the second case, as this would allow to type
     ambiguous records}
