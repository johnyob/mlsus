...
equirecursive types.} $\semenv$ maps each type variable $\tv$ to a ground types $\gt \in \Ground$ (the type guessed for $\tv$), and each term variable $\x$ to sets of ground types $\gabs \subseteq \Ground$ (the set of ground instances of the type scheme guessed for $\x$).
%
We write $\semenv\where{\tv \is \gt}$ and $\semenv\where{\x \is \gabs}$ for the extension of $\semenv$ with a new binding. For a type $\t$, we write $\semenv(\t)$ for the ground type obtained by substitution.
...
\begin{mathparfig}[htpb!]
  ...
  \infer[Papp]{
   \semenv\where{\tv \is \semenv(\t)} \th \ueqs \where \ren \\
   \greg[\semenv \compose \ren] \tv {\semenv(\t)} \tvs \gts \in \semenv(\x)
   \\
  }{
   \semenv \th \cpapp \x \ren \ueqs \t
  }
