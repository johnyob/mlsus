...
module Convert = struct
  ...
  and core_scheme_to_type_scheme ~env scheme : Type.t =
    let { scheme_quantifiers; scheme_body } = scheme.it in
    ...
    Type.defunctionalized_scheme { scheme_quantifiers; scheme_body }
  ;;
  ...
  and type_scheme_expr ~env { scheme_quantifiers; scheme_body } : Type.t =
    let env, scheme_quantifiers =
      List.fold_map scheme_quantifiers ~init:env ~f:(fun env type_var ->
...
module Expression = struct
  ...
  let rec infer_exp ~(env : Env.t) (exp : expression) (exp_type : Type.Var.t) =
      ...
      (match scheme_annot with
       | None ->
         Mlsus_error.(
           ...
           @@ bug_s
                ~here:[%here]
                [%message "Implicit poltype boxing is not supported yet"])
       | Some core_scheme ->
    ...
    | Exp_inst exp ->
      ...
      &~ Type.(var exp_type' =~ poly (var closure_type @% var scm_type))
      &~ lower scm_type
      &~ match_
           scm_type
           ~closure:[ scm_type; closure_type; exp_type ]
           ~with_:(function
             | App _ | Spine _ ->
               Mlsus_error.(
                 raise
                 @@ bug_s
                      ~here:[%here]
                      [%message "Kind mismatch, expected hd but got */sp"])
             | (Head (Arrow | Tuple _ | Poly | Constr _) | Rigid_var) as matchee ->
               let type_head =
                 match matchee with
                 | Head Arrow -> `Arrow
                 | Head (Tuple _) -> `Tuple
                 ...
                 | Rigid_var -> `Rigid_var
                 | Head (Constr _) ->
                   (* Map constructor ident to name for errors *)
                   `Constr
                 | _ -> assert false
               in
               ff (Mlsus_error.polytype_inst_mismatched_type ~range:exp.range ~type_head)
             | Head (Scheme scheme_skeleton) ->
               inst_scm
                 { scheme_skeleton; scheme_closure = Type.var closure_type }
                 Type.(var exp_type))
           ~else_:(fun () ->
             exists' ~id_source
             @@ fun mono ->
             Type.(
               var closure_type @% var scm_type =~ defunctionalized_scheme ([] @. var mono)))
